...
function _signed_stoich_term(stoich, expr)
    return if stoich isa SymbolicT
        stoich * expr
    else
...
function assemble_oderhs(
        rs, ispcs; combinatoric_ratelaws = true, remove_conserved = false,
    ...
    for (rxidx, rx) in enumerate(get_rxs(rs))
        ...
        !(
            (physical_scales === nothing) ||
                (physical_scales[rxidx] in (PhysicalScale.ODE, PhysicalScale.SDE))
        ) && continue
...
function assemble_drift(
        rs, ispcs; combinatoric_ratelaws = true, as_odes = true,
        include_zero_odes = true, remove_conserved = false, physical_scales = nothing,
        expand_catalyst_funs = true, use_jump_ratelaws = false
    )
    rhsvec = assemble_oderhs(
        rs, ispcs; combinatoric_ratelaws, remove_conserved,
...
"""
function foreach_noise_coeff(
        f, rx, species_to_idx, nps, depspec_submap;
    ...
    for (spec, stoich) in rx.netstoich
        ...
        if !haskey(species_to_idx, spec)
            error(
                "Species $spec appears in reaction $rx but is not in the independent species list. " *
...
function assemble_diffusion(
        rs, sts, ispcs; combinatoric_ratelaws = true,
    ...
    for (j, rx) in enumerate(get_rxs(rs))
        foreach_noise_coeff(
            rx, species_to_idx, nps, depspec_submap;
...
function create_sde_brownians(scales)
    ...
    return brownian_vars, brownian_map
end
...
"""
function add_noise_to_rhs!(
        rhsvec, rs, ispcs, brownian_map;
        combinatoric_ratelaws = true, remove_conserved = false,
        expand_catalyst_funs = true, use_jump_ratelaws = false
    )
    nps = get_networkproperties(rs)
...
function ismassaction(
        rx, rs; rxvars = get_variables(rx.rate),
...
@inline function makemajump(rx; combinatoric_ratelaw = true)
    ...
    return MassActionJump(
        Num(rate), reactant_stoch, net_stoch, scale_rates = false,
        useiszero = false
    )
end
...
function get_depgraph(rs)
    ...
    return eqeq_dependencies(jdeps, vdeps).fadjlist
end
...
function assemble_jumps(
        rs; combinatoric_ratelaws = true, physical_scales = nothing,
    ...
    return reduce(vcat, (meqs, ceqs, veqs); init = JumpType[])
end
...
function addconstraints!(
        eqs, rs::ReactionSystem, ists, ispcs; remove_conserved = false,
    ...
    return eqs, sts, ps, obs, ics, initeqs
end
...
function spatial_convert_err(rs::ReactionSystem, systype)
    return isspatial(rs) && error("Conversion to $systype is not supported for spatial networks.")
end
...
function hybrid_model(
    ...
    if has_continuous
        rhsvec = assemble_oderhs(
            flatrs, ispcs; combinatoric_ratelaws, remove_conserved,
        ...
        if has_rxn_sde
            ...
            add_noise_to_rhs!(
                rhsvec, flatrs, ispcs, brownian_map;
                combinatoric_ratelaws, remove_conserved, expand_catalyst_funs,
                use_jump_ratelaws
            )
    ...
    rxn_jumps = JumpType[]
    if has_rxn_jump
        rxn_jumps = assemble_jumps(
            flatrs; combinatoric_ratelaws, expand_catalyst_funs,
...
"""
function ode_model(
        rs::ReactionSystem; name = nameof(rs),
        combinatoric_ratelaws = get_combinatoric_ratelaws(rs),
    ...
    if !isempty(MT.brownians(flatrs))
        error(
            """Cannot convert ReactionSystem with coupled brownian noise to a pure ODE system.
    ...
    if !isempty(MT.jumps(flatrs))
        error(
            """Cannot convert ReactionSystem with coupled jumps to a pure ODE system.
    ...
    if !isempty(MT.poissonians(flatrs))
        error(
            """Cannot convert ReactionSystem with poissonians to a pure ODE system.
...
function _mark_species_bc(node)
    ...
    return MT.setmetadata(node, VariableBCSpecies, true)
end
...
"""
function system_to_reactionsystem(
        sys::MT.AbstractSystem;
        name = nameof(sys),
    ...
    us = [
        isspecies(u) && !isbc(u) ? MT.setmetadata(u, VariableBCSpecies, true) : u
            for u in us
    ]
    eqs = [
        replacenode(eq.lhs, _mark_species_bc) ~ replacenode(eq.rhs, _mark_species_bc)
            for eq in eqs
    ]
...
function nonlinear_convert_differentials_check(rs::ReactionSystem)
    for eq in filter(is_diff_equation, equations(rs))
        ...
        if hasnode(Symbolics.is_derivative, eq.rhs) ||
            ...
            error(
                "You are attempting to convert a `ReactionSystem` coupled with differential equations to a nonlinear system. However, some of these differentials are not of the form `D(x) ~ ...` where:
...
function sde_model(
    ...
    if !isempty(MT.jumps(flatrs))
        error(
            """Cannot convert ReactionSystem with coupled jumps to a pure SDE system.
            Found $(length(MT.jumps(flatrs))) jump(s).
    ...
    if !isempty(MT.poissonians(flatrs))
        error(
            """Cannot convert ReactionSystem with poissonians to a pure SDE system.
...
function detect_scale_types(scales)
    ...
    return (has_ode, has_sde, has_jump)
end
...
function jump_model(
    ...
    if !isempty(MT.poissonians(flatrs))
        error(
            """Cannot convert ReactionSystem with unprocessed poissonians to a pure Jump system via jump_model.
    ...
    if !isempty(non_rxn_eqs)
        error(
            """Cannot convert ReactionSystem with ODE, SDE, or algebraic equations to a pure Jump system.
...
"""
    ODEProblem(rs::ReactionSystem, u0, tspan, p = NullParameters(), args...;
...
Create an `ODEProblem` from a [`ReactionSystem`](@ref).

The reaction system is first converted with [`ode_model`](@ref), completed or
compiled, and then passed to SciMLBase's `ODEProblem` constructor. Initial
conditions `u0` and parameters `p` may be supplied as Catalyst/SciML symbolic
maps, such as `[:S => 1.0]` and `[:k => 0.1]`.

# Arguments
- `rs`: Reaction system to simulate as ordinary differential equations.
- `u0`: Initial condition map or vector accepted by ModelingToolkit/SciMLBase.
- `tspan`: Time span for the ODE problem.
- `p`: Parameter map or vector. If omitted, only `u0` is used for problem
  conditions.

# Keyword Arguments
- `combinatoric_ratelaws`: Use combinatoric mass-action rate laws.
- `include_zero_odes`: Include zero right-hand-side equations.
- `remove_conserved`: Eliminate conservation laws before problem construction.
- `mtkcompile`: Apply `ModelingToolkitBase.mtkcompile` before constructing the
  problem. This is required for algebraic equations.
- `use_jump_ratelaws`: Use stochastic jump rate laws for ODE drift terms.
- `kwargs...`: Forwarded to the underlying `ODEProblem` constructor.

# Returns
An `ODEProblem` whose states and parameters are ordered by the compiled
...
oprob = ODEProblem(rn, [:S => 1.0, :P => 0.0], (0.0, 10.0), [:k => 0.2])
```
"""
function DiffEqBase.ODEProblem(
        rs::ReactionSystem, u0, tspan,
    ...
    osys = ode_model(
        rs; name, combinatoric_ratelaws, include_zero_odes, checks,
        remove_conserved, expand_catalyst_funs, use_jump_ratelaws
    )
...
"""
    NonlinearProblem(rs::ReactionSystem, u0, p = NullParameters(), args...;
        name = nameof(rs), combinatoric_ratelaws = get_combinatoric_ratelaws(rs),
        remove_conserved = false, checks = false, check_length = false,
        mtkcompile = remove_conserved, all_differentials_permitted = false,
        kwargs...)

Create a `NonlinearProblem` for steady states of a [`ReactionSystem`](@ref).

The reaction system is converted with [`ss_ode_model`](@ref), completed or
compiled, and then passed to SciMLBase's nonlinear problem constructor.

# Arguments
- `rs`: Reaction system whose steady-state equations should be solved.
- `u0`: Initial nonlinear solve guess.
- `p`: Parameter map or vector.

# Keyword Arguments
- `combinatoric_ratelaws`: Use combinatoric mass-action rate laws.
- `remove_conserved`: Replace one steady-state equation per conservation law by
  its conserved quantity.
- `mtkcompile`: Apply structural simplification before problem construction.
- `all_differentials_permitted`: Permit differential terms while building the
  steady-state system.
- `kwargs...`: Forwarded to the underlying `NonlinearProblem` constructor.

# Returns
A `NonlinearProblem` representing the steady-state equations of `rs`.
...
nlprob = NonlinearProblem(rn, [:X => 1.0], [:p => 2.0, :d => 1.0])
```
"""
function DiffEqBase.NonlinearProblem(
        rs::ReactionSystem, u0,
        p = DiffEqBase.NullParameters(), args...;
        name = nameof(rs), combinatoric_ratelaws = get_combinatoric_ratelaws(rs),
    ...
    nlsys = ss_ode_model(
        rs; name, combinatoric_ratelaws, checks, all_differentials_permitted,
        remove_conserved, expand_catalyst_funs
    )
    nlsys = mtkcompile ? MT.mtkcompile(nlsys) : complete(nlsys)
    prob_cond = (p isa DiffEqBase.NullParameters) ? u0 : merge(Dict{Any, Any}(u0), Dict{Any, Any}(p))
    return NonlinearProblem(
        nlsys, prob_cond, args...; check_length,
        kwargs...
    )
end

"""
    SDEProblem(rs::ReactionSystem, u0, tspan, p = NullParameters(), args...;
        name = nameof(rs), combinatoric_ratelaws = get_combinatoric_ratelaws(rs),
...
Create an `SDEProblem` from a [`ReactionSystem`](@ref).

The reaction system is converted with [`sde_model`](@ref). Catalyst can build
chemical Langevin noise from reactions and can also route user-declared
Brownian terms through `ModelingToolkitBase.mtkcompile`.

# Arguments
- `rs`: Reaction system to simulate as stochastic differential equations.
- `u0`: Initial condition map or vector.
- `tspan`: Time span for the SDE problem.
- `p`: Parameter map or vector.

# Keyword Arguments
- `combinatoric_ratelaws`: Use combinatoric mass-action rate laws.
- `remove_conserved`: Eliminate conservation laws before problem construction.
- `mtkcompile`: Apply structural simplification. This is required for algebraic
  equations and user-declared Brownian variables.
- `use_legacy_noise`: Use Catalyst's legacy chemical Langevin noise matrix path.
- `use_jump_ratelaws`: Use stochastic jump rate laws for drift and diffusion.
- `kwargs...`: Forwarded to the underlying `SDEProblem` constructor.

# Returns
An `SDEProblem` for the stochastic dynamics encoded by `rs`.
...
sprob = SDEProblem(rn, [:X => 10.0], (0.0, 1.0), [:p => 2.0, :d => 1.0])
```
"""
function DiffEqBase.SDEProblem(
        rs::ReactionSystem, u0, tspan,
        p = DiffEqBase.NullParameters(), args...;
        name = nameof(rs), combinatoric_ratelaws = get_combinatoric_ratelaws(rs),
        include_zero_odes = true, checks = false, check_length = false,
    ...
    flatrs = Catalyst.flatten(rs)
    sde_sys = sde_model(
        flatrs; name, combinatoric_ratelaws, expand_catalyst_funs,
    ...
    if needs_mtkcompile
        ...
        return SDEProblem(
            sde_sys, prob_cond, tspan, args...; check_length,
...
"""
    JumpProblem(rs::ReactionSystem, u0, tspan, p = NullParameters();
        name = nameof(rs), combinatoric_ratelaws = get_combinatoric_ratelaws(rs),
        expand_catalyst_funs = true, save_positions = (true, true),
        checks = false, kwargs...)

Create a `JumpProblem` from a pure-jump [`ReactionSystem`](@ref).

Use this constructor for stochastic chemical kinetics models. For systems that
mix jump-scale reactions with ODE-scale or SDE-scale reactions, use
[`HybridProblem`](@ref) instead.

# Arguments
- `rs`: Reaction system to convert with [`jump_model`](@ref).
- `u0`: Initial condition map or vector.
- `tspan`: Time span for the jump problem.
- `p`: Parameter map or vector.

# Keyword Arguments
- `combinatoric_ratelaws`: Use combinatoric mass-action propensities.
- `save_positions`: Save solution values before and after variable-rate jumps.
- `kwargs...`: Forwarded to the underlying `JumpProblem` constructor.

# Returns
A `JumpProblem` whose internal problem is generated from `rs`.
...
jprob = JumpProblem(rn, [:X => 10], (0.0, 1.0), [:p => 2.0, :d => 1.0])
```
"""
function JumpProcesses.JumpProblem(
    ...
    jsys = complete(
        jump_model(
            rs; name, combinatoric_ratelaws, checks,
...
function HybridProblem(
    ...
    sys = hybrid_model(
        flatrs; name, physical_scales, default_scale,
...
"""
    SteadyStateProblem(rs::ReactionSystem, u0, p = NullParameters(), args...;
        check_length = false, name = nameof(rs),
        combinatoric_ratelaws = get_combinatoric_ratelaws(rs),
        remove_conserved = false, include_zero_odes = true, checks = false,
        expand_catalyst_funs = true, mtkcompile = false, kwargs...)

Create a `SteadyStateProblem` from a [`ReactionSystem`](@ref).

This constructor builds an ODE representation with [`ode_model`](@ref) and then
constructs a SciML steady-state problem from it.

# Arguments
- `rs`: Reaction system whose ODE steady states should be computed.
- `u0`: Initial condition or steady-state guess.
- `p`: Parameter map or vector.
...
- `remove_conserved`: Eliminate conservation laws before problem construction.
- `mtkcompile`: Apply structural simplification before problem construction.
- `kwargs...`: Forwarded to the underlying `SteadyStateProblem` constructor.

# Returns
A `SteadyStateProblem` for ODE-based steady-state solving.
...
ssprob = SteadyStateProblem(rn, [:X => 1.0], [:p => 2.0, :d => 1.0])
```
"""
function DiffEqBase.SteadyStateProblem(
        rs::ReactionSystem, u0,
        p = DiffEqBase.NullParameters(), args...;
        check_length = false, name = nameof(rs),
        combinatoric_ratelaws = get_combinatoric_ratelaws(rs),
...
function symmap_to_varmap(sys, symmap::Dict{Symbol, T}) where {T}
    return Dict(_symbol_to_var(sys, sym) => val for (sym, val) in symmap)
end

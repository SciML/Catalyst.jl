...
function numparams(network)
    Base.depwarn("`numparams` is depreciated, please use `length(ModelingToolkit.parameters)` for the total number of parameters across all systems and subsystems, or `numreactionparams` for the number of parameters within system and subsystem `Reaction`s.",
                 :params; force = true)
    return length(parameters(network))
end
...
function reactionparams(network)
    ...
    return unique([ps; reduce(vcat, map(sys -> species(sys, reactionparams(sys)), systems))])
end
...
function reactionparamsmap(network)
    return Dict(p => i for (i, p) in enumerate(reactionparams(network)))
end
...
function numreactionparams(network)
    return length(reactionparams(network))
end
...
function substoichmat(rn::ReactionSystem; sparse::Bool = false)
    ...
    return sparse ? substoichmat(SparseMatrixCSC{Int,Int}, rn) : substoichmat(Matrix{Int}, rn)
end
...
function prodstoichmat(rn::ReactionSystem; sparse = false)
    ...
    return sparse ? prodstoichmat(SparseMatrixCSC{Int,Int}, rn) : prodstoichmat(Matrix{Int}, rn)
end
...
symmap_to_varmap(sys, symmap::AbstractArray{Pair{Symbol,T}}) where {T} = [_symbol_to_var(sys, sym) => val
                                                                          for (sym, val) in symmap]

symmap_to_varmap(sys, symmap::Dict{Symbol,T}) where {T} = Dict(_symbol_to_var(sys, sym) => val
                                                               for (sym, val) in symmap)
...
function subnetworkmapping(linkageclass, allrxs, complextorxsmap, p)
    ...
    return rxs, specs, newps   # reactions and species involved in reactions of subnetwork
end
...
function (==)(rn1::ReactionSystem, rn2::ReactionSystem)
    ...
    return isequal_ignore_names(rn1, rn2)
end
...
function addspecies!(network::ReactionSystem, s::Num; disablechecks = false)
    return addspecies!(network, value(s); disablechecks = disablechecks)
end
...
function addparam!(network::ReactionSystem, p::Num; disablechecks = false)
    return addparam!(network, value(p); disablechecks = disablechecks)
end
...
function validate(rs::ReactionSystem, info::String = "")
    ...
    for spec in specs
        if get_unit(spec) != specunits
            validated = false
            @warn(string("Species are expected to have units of ", specunits, " however, species ",
                         spec, " has units ", get_unit(spec), "."))
        end
    end

...
Notes:
- `ReactionSystem`s with non-`ReactionSystem` sub-systems (e.g. non-reaction `System`s) cannot be saved.
...
function save_reactionsystem(
    ...
    if !isempty(MT.get_tstops(rn)) || (!isempty(get_systems(rn)) && !isempty(MT.symbolic_tstops(rn)))
        error(
            "Serialization of ReactionSystems with tstops is not yet supported. " *
...
function get_full_system_string(rn::ReactionSystem, annotate::Bool, top_level::Bool)
    ...
    file_text, has_brownians = push_field(
        file_text, rn, annotate,
        top_level, BROWNIAN_TYPE_FS
    )
    file_text, has_jumps = push_field(
        file_text, rn, annotate,

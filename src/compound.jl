...
macro compound(species_expr, arr_expr...)
    ...
    setcomponents_expr = :($(species_name) = ModelingToolkit.setmetadata($(species_name), Catalyst.CompoundData, $arr))
...
#     array = eval(arr)
#     coefficients = [Symbolics.arguments(Symbolics.value(element))[1] for element in array]
#     setcoeff_expr = :($(species_name) = ModelingToolkit.setmetadata($(species_name), Catalyst.CompoundCoefficients, $coefficients))
#     escaped_setcoeff_expr = esc(setcoeff_expr)
...
#         # append the extracted variables to the new array
#         append!(components, extracted)
#     end

#     # Construct the expression to set the components metadata
#     setcomponents_expr = :($(species_name) = ModelingToolkit.setmetadata($(species_name), Catalyst.CompoundComponents, $components))

#     # Ensure the expression is evaluated in the correct scope by escaping it
#     escaped_setcomponents_expr = esc(setcomponents_expr)

#     # Return a block that contains the escaped expressions
#     return Expr(:block, escaped_species_expr, escaped_setmetadata_expr, escaped_setcomponents_expr,escaped_setcoeff_expr)
# end
...
#     coefficients = [Symbolics.arguments(Symbolics.value(element))[1] for element in $arr]
#     setcoeff_expr = :($(species_name) = ModelingToolkit.setmetadata($(species_name), CompoundCoefficients, $coefficients))
#     escaped_setcoeff_expr = esc(setcoeff_expr)
...
#     # Return a block that contains the escaped expressions
#     return Expr(:block, escaped_species_expr, escaped_setmetadata_expr, escaped_setcoeff_expr)
# end     

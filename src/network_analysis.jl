...
function substitutevals(rn::ReactionSystem, map::Dict, syms, symexprs)
    ...
    return vals = [substitute(expr, map) for expr in symexprs]
end
...
function cache_conservationlaw_eqs!(rn::ReactionSystem, N::AbstractMatrix, col_order)
    ...
    for (i, depidx) in enumerate(depidxs)
        ...
        (scaleby != 0) || error(
            "Error, found a zero in the conservation law matrix where "
                *
...
function conservationlaw_errorcheck(rs, pre_varmap)
    ...
    return isempty(conservedequations(Catalyst.flatten(rs))) ||

...
# Creates the hopping constants from a discrete problem and a discrete space reaction system.
function make_hopping_constants(dprob::DiscreteProblem, dsrs::DiscreteSpaceReactionSystem)
    ...
    edge_array = [Pair{Int64, Int64}[] for _1 in 1:num_species(dsrs), _2 in 1:num_verts(dsrs)]
    for e in edge_iterator(dsrs), s_idx in 1:num_species(dsrs)
...
# Creates a SpatialMassActionJump struct from a (spatial) DiscreteProblem and a DiscreteSpaceReactionSystem.
...
function make_spatial_majumps(dprob, dsrs::DiscreteSpaceReactionSystem)
    ...
    num_rxs = length(reactions(reactionsystem(dsrs)))
    u_rates = Vector{Float64}(undef, num_rxs - count(is_spatials))
    s_rates = Matrix{Float64}(undef, count(is_spatials), num_verts(dsrs))
    ...
    for (is_spat, rx) in zip(is_spatials, reactions(reactionsystem(dsrs)))
        is_spat && continue
        u_rates[cur_rx] = compute_vertex_value(rx.rate, dsrs; ps = dprob.p)[1]

...
struct LatticeTransportODEFunction{P, Q, R, S, T}
    ...
    not identical across the space). Each index corresponds to its position in the full parameter
    vector (`parameters(dsrs)`).
    """
    ...
    vertex parameter whose value is not identical across the space). The `setp` function at index
    ...
    Each entry is a pair from (the index of) the transported species (in the `species(dsrs)` vector)
    ...
    """An iterator over all the edges of the space."""
...
function make_mtk_ps_structs(ps, dsrs, heterogeneous_vert_p_idxs)
    p_dict = Dict(ps)
...
function make_t_types_and_leaving_rates(transport_rates, dsrs)
    t_rate_idx_types = [size(tr[2]) == (1, 1) for tr in transport_rates]
    leaving_rates = zeros(length(transport_rates), num_verts(dsrs))
...
function DiffEqBase.ODEProblem(dsrs::DiscreteSpaceReactionSystem, u0_in, tspan,
    ...
    u0_in = symmap_to_varmap(dsrs, u0_in)
    p_in = symmap_to_varmap(dsrs, p_in)
    ...
    u0 = dspace_process_u0(u0_in, species(dsrs), dsrs)
    ...
    ps = [pval_dict[p] for p in parameters(dsrs)]
...
function build_jac_prototype(ns_jac_prototype::SparseMatrixCSC{Float64, Int64},
    ...
    for vert in 1:num_verts(dsrs)
        for n in 1:length(ns_i_idxs)
            i_idxs[idx] = get_index(vert, ns_i_idxs[n], num_species(dsrs))
            j_idxs[idx] = get_index(vert, ns_j_idxs[n], num_species(dsrs))
            idx += 1
        end
    end
    ...
    for e in edge_iterator(dsrs)
        ...
        for s_idx in trans_only_species
            i_idxs[idx] = get_index(e[1], s_idx, num_species(dsrs))
        ...
        for s_idx in trans_species
            i_idxs[idx] = get_index(e[1], s_idx, num_species(dsrs))
            j_idxs[idx] = get_index(e[2], s_idx, num_species(dsrs))
            idx += 1
        end
    end
    ...
    set_nonzero && set_jac_transport_values!(jac_prototype, transport_rates, dsrs)
...
function set_jac_transport_values!(jac_prototype, transport_rates, dsrs)
    for (s, rates) in transport_rates, e in edge_iterator(dsrs)

        idx_src = get_index(e[1], s, num_species(dsrs))
        idx_dst = get_index(e[2], s, num_species(dsrs))

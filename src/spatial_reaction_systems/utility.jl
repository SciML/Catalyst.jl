...
# Defines _symbol_to_var, but where the system is a dsrs. Required to make symmapt_to_varmap to work.
function _symbol_to_var(dsrs::DiscreteSpaceReactionSystem, sym)
    # Checks if sym is a parameter.
    p_idx = findfirst(sym == p_sym for p_sym in MT.getname.(parameters(dsrs)))
    isnothing(p_idx) || return parameters(dsrs)[p_idx]

    # Checks if sym is a species.
    s_idx = findfirst(sym == s_sym for s_sym in MT.getname.(species(dsrs)))
    isnothing(s_idx) || return species(dsrs)[s_idx]

    error("Could not find property parameter/species $sym in discrete space reaction system.")
end
...
function dspace_process_u0(u0_in, u0_syms::Vector, dsrs::DiscreteSpaceReactionSystem)
    ...
    u0 = dspace_process_input(u0_in, u0_syms)

    # Species' initial condition values can be given in different forms (also depending on the space).
...
function dspace_process_p(ps_in, ps_vertex_syms::Vector,
    ...
    ps = dspace_process_input(ps_in, [ps_vertex_syms; ps_edge_syms])
    ...
    vert_ps = vertex_value_map(vert_ps, dsrs)
    edge_ps = edge_value_map(edge_ps, dsrs)
...
function dspace_process_input(input::Dict{<:Any, T}, syms::Vector) where {T}
    # Error checks
...
function dspace_process_input(input, syms::Vector)
    if ((input isa Vector) || (input isa Tuple)) && all(entry isa Pair for entry in input)
        return dspace_process_input(Dict(input), syms)
    end
...
function vertex_value_map(values, dsrs::DiscreteSpaceReactionSystem)
    isempty(values) && (return Pair{SymbolicT, Vector{Float64}}[])
    return [entry[1] => vertex_value_form(entry[2], dsrs, entry[1]) for entry in values]
end

# Converts the values for an individual species/vertex parameter to its correct vector form.
function vertex_value_form(values, dsrs::DiscreteSpaceReactionSystem, sym::SymbolicT)
    # If the value is a scalar (i.e. uniform across the space), return it in vector form.
    (values isa AbstractArray) || (return [values])

    # If the value is a vector (something all three space types accept).
    if values isa Vector
        ...
        if has_grid_dspace(dsrs) && (size(values) == grid_size(dsrs))
            return vertex_value_form(values, num_verts(dsrs), dspace(dsrs), sym)
        end
        ...
        if (length(values) != num_verts(dsrs))
            throw(ArgumentError("You have provided ($(length(values))) values for $sym. This is not equal to the number of vertices ($(num_verts(dsrs)))."))
        end
        return values
    end
...
# Converts values to the correct vector form for a Cartesian grid space.
function vertex_value_form(values::AbstractArray, num_verts::Int64,
        dspace::CartesianGridRej{N, T}, sym::SymbolicT) where {N, T}
    if size(values) != dspace.dims
        throw(ArgumentError("The values for $sym did not have the same format as the space. Expected a $(dspace.dims) array, got one of size $(size(values))"))
    end
    ...
    return [values[fspat_idx] for fspat_idx in 1:num_verts]
end

# Converts values to the correct vector form for a masked grid space.
function vertex_value_form(values::AbstractArray, num_verts::Int64,
        dspace::Array{Bool, T}, sym::SymbolicT) where {T}
    if size(values) != size(dspace)
        throw(ArgumentError("The values for $sym did not have the same format as the space. Expected a $(size(dspace)) array, got one of size $(size(values))"))
    end

    # Pre-declares a vector with the values in each vertex (return_values).
    # Loops through the space and the values, adding these to the return_values.
...
function edge_value_map(values, dsrs::DiscreteSpaceReactionSystem)
    isempty(values) && (return Pair{SymbolicT, SparseMatrixCSC{Float64, Int64}}[])
    return [entry[1] => edge_value_form(entry[2], dsrs, entry[1]) for entry in values]
end

# Converts the values for an individual edge parameter to its correct sparse matrix form.
function edge_value_form(values, dsrs::DiscreteSpaceReactionSystem, sym)
    # If the value is a scalar (i.e. uniform across the space), return it in sparse matrix form.
    (values isa SparseMatrixCSC) || (return sparse([1], [1], [values]))

    # Error checks.
    if nnz(values) != num_edges(dsrs)
        throw(ArgumentError("You have provided ($(nnz(values))) values for $sym. This is not equal to the number of edges ($(num_edges(dsrs)))."))
    end
    if !all(Base.isstored(values, e[1], e[2]) for e in edge_iterator(dsrs))
...
# The species is represented by its index (in species(dsrs).
...
function make_sidxs_to_transrate_map(vert_ps::Vector{Pair{R, Vector{T}}},
        ...
        dsrs::DiscreteSpaceReactionSystem) where {R, S, T}
    ...
    transport_rates_speciesmap = compute_all_transport_rates(p_val_dict, dsrs)
    return Pair{Int64, SparseMatrixCSC{T, Int64}}[speciesmap(reactionsystem(dsrs))[spat_rates[1]] => spat_rates[2]
                                                  for spat_rates in
...
function compute_all_transport_rates(p_val_dict, dsrs::DiscreteSpaceReactionSystem)
    ...
    unsorted_rates = [s => compute_transport_rates(s, p_val_dict, dsrs)
                      for s in spatial_species(dsrs)]

    # Sorts all the species => rate pairs according to their species index in species(dsrs).
    return sort(unsorted_rates; by = rate -> findfirst(isequal(rate[1]), species(dsrs)))
end
...
function compute_transport_rates(s::SymbolicT, p_val_dict, dsrs::DiscreteSpaceReactionSystem)
    # Find parameters involved in the rate and create a function evaluating the rate law.
    rate_law = get_transport_rate_law(s, dsrs)
    ...
    if all(size(p_val_dict[p]) == (1, 1) for p in relevant_ps)
        ...
        transport_rates = spzeros(num_verts(dsrs), num_verts(dsrs))
...
function get_transport_rate_law(s::SymbolicT, dsrs::DiscreteSpaceReactionSystem)
    rates = filter(sr -> isequal(s, sr.species), spatial_reactions(dsrs))
...
function compute_vertex_value(exp, dsrs::DiscreteSpaceReactionSystem; u = [], ps = [])
    ...
    if any(any(isequal(sym) in edge_parameters(dsrs)) for sym in relevant_syms)
    ...
    return [exp_func([get_vertex_value(value_dict[sym], vert_idx) for sym in relevant_syms]...)
            for vert_idx in 1:num_verts(dsrs)]
end

...
function vertex_value_form(
        ...
        dspace::CartesianGridRej{N, T}, sym::SymbolicT
    ) where {N, T}
...
function vertex_value_form(
        values::AbstractArray, num_verts::Int64,
        dspace::Array{Bool, T}, sym::SymbolicT
    ) where {T}
...
function make_sidxs_to_transrate_map(
    ...
    return Pair{Int64, SparseMatrixCSC{T, Int64}}[
        speciesmap(reactionsystem(dsrs))[spat_rates[1]] => spat_rates[2]
            for spat_rates in
...
function compute_all_transport_rates(p_val_dict, dsrs::DiscreteSpaceReactionSystem)
    ...
    unsorted_rates = [
        s => compute_transport_rates(s, p_val_dict, dsrs)

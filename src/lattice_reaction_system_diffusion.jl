...
struct LatticeReactionSystem # <: MT.AbstractTimeDependentSystem # Adding this part messes up show, disabling me from creating LRSs
    ...
    function LatticeReactionSystem(rs, spatial_reactions, lattice::DiGraph)
        return new(rs, spatial_reactions, lattice, unique(getfield.(spatial_reactions, :rate)), length(vertices(lattice)), length(species(rs)))
    end
...
function DiffEqBase.ODEProblem(lrs::LatticeReactionSystem, u0_in, tspan,
    ...
    u0 = [get_component_value(u0, species, comp) for comp in 1:lrs.nC for species in 1:lrs.nS]
...
split_parameters(ps::Vector{<:Pair}, spatial_params::Vector{Symbol}) = (filter(p->!(p[1] in spatial_params), ps), filter(p->p[1] in spatial_params, ps))
split_parameters(ps::Vector{<:Number}, spatial_params::Vector{Symbol}) = (ps[1:length(ps)-length(spatial_params)], ps[length(ps)-length(spatial_params)+1:end])
# Sorts a parameter (or species) vector along parameter (or species) index, and remove the Symbol in the pair.
resort_values(values::Vector{<:Pair}, symbols::Vector{Symbol}) = last.(sort(values; by=val -> findfirst(val[1].==symbols)))
...
function build_odefunction(lrs::LatticeReactionSystem, pV, pE, use_jac::Bool, sparse::Bool)
    ...
    diffusion_species = Int64[findfirst(s .== [Symbol(s.f) for s in species(lrs.rs)]) for s in getfield.(lrs.spatial_reactions,:species)]

    f = build_f(ofunc, pV, pE, diffusion_species, lrs)
    jac_prototype = (use_jac ? build_jac_prototype(ofunc_sparse.jac_prototype, pE, diffusion_species, lrs; set_nonzero=true) : nothing)
    jac = (use_jac ? build_jac(ofunc, pV, pE, diffusion_species, lrs, jac_prototype; sparse=sparse) : nothing)
...
function build_f(ofunc::SciMLBase.AbstractODEFunction{true}, pV, pE, diffusion_species::Vector{Int64}, lrs::LatticeReactionSystem)
    leaving_rates = zeros(length(diffusion_species), lrs.nC)
    for (s_idx,species) in enumerate(diffusion_species), (e_idx, e) in enumerate(edges(lrs.lattice))
    ...
    return function(du, u, p, t)
        ...
        for comp_i::Int64 in 1:lrs.nC            
            ofunc((@view du[get_indexes(comp_i,lrs.nS)]), (@view u[get_indexes(comp_i,lrs.nS)]), make_p_vector!(p_base, p, p_update_idx, comp_i), t)
        end

        for (s_idx::Int64,species::Int64) in enumerate(diffusion_species)
            for comp_i::Int64 in 1:lrs.nC
                du[get_index(comp_i,species,lrs.nS)] -= leaving_rates[s_idx, comp_i] * u[get_index(comp_i,species,lrs.nS)]
            end
            for (e_idx::Int64,edge::Graphs.SimpleGraphs.SimpleEdge{Int64}) in enumerated_edges
                du[get_index(edge.dst,species,lrs.nS)] +=  get_component_value(pE, s_idx, e_idx) * u[get_index(edge.src,species,lrs.nS)]
            end
        end
    end
end

function build_jac(ofunc::SciMLBase.AbstractODEFunction{true}, pV, pE, diffusion_species::Vector{Int64}, lrs::LatticeReactionSystem, jac_prototype::SparseMatrixCSC{Float64, Int64}; sparse=true)
    p_base = deepcopy(first.(pV))
    p_update_idx = (p_base isa Vector) ? findall(typeof.(p_base) .== Vector{Float64}) : []
    ...
    if sparse 
            ...
            # Updates for non-spatial reactions.
            for comp_i::Int64 in 1:lrs.nC
                ofunc.jac((@view J[get_indexes(comp_i,lrs.nS),get_indexes(comp_i,lrs.nS)]), (@view u[get_indexes(comp_i,lrs.nS)]), make_p_vector!(p_base, p, p_update_idx, comp_i), t)
            end
        end
    else
...
function build_jac_prototype(ns_jac_prototype::SparseMatrixCSC{Float64, Int64}, pE, diffusion_species::Vector{Int64}, lrs::LatticeReactionSystem; set_nonzero=false)   
    only_diff = [(s in diffusion_species) && !Base.isstored(ns_jac_prototype,s,s) for s in 1:lrs.nS]

    # Declares sparse array content.
    J_colptr = fill(1,lrs.nC*lrs.nS+1)
    J_nzval = fill(0.0, lrs.nC*(nnz(ns_jac_prototype) + count(only_diff)) + length(edges(lrs.lattice))*length(diffusion_species))
    ...
    for comp in 1:lrs.nC, s in 1:lrs.nS
        ...
        # Column values.
        local_elements = in(s, diffusion_species)*(length(lrs.lattice.fadjlist[comp]) + only_diff[s])
        diffusion_elements = -(ns_jac_prototype.colptr[s+1:-1:s]...)
        J_colptr[col_idx+1] = J_colptr[col_idx] + local_elements + diffusion_elements
        
        # Row values.
        rows = ns_jac_prototype.rowval[ns_jac_prototype.colptr[s]:ns_jac_prototype.colptr[s+1]-1] .+ (comp-1)*lrs.nS
        if in(s, diffusion_species)
            ...
            rows = vcat(diffusion_rows[1:split_idx-1],rows,diffusion_rows[split_idx:end])
            if only_diff[s]
    ...
    if set_nonzero
        ...
        for (s_idx,s) in enumerate(diffusion_species), (e_idx,edge) in enumerate(edges(lrs.lattice))
            ...
            val_idx_src = col_start + findfirst(column_view .== get_index(edge.src, s_idx, lrs.nS)) - 1
            J_nzval[val_idx_src] -= get_component_value(pE, s_idx, e_idx)

            # Updates the destination value.
            val_idx_dst = col_start + findfirst(column_view .== get_index(edge.dst, s_idx, lrs.nS)) - 1
...
get_component_value(vals::Matrix{Float64}, component::Int64, place::Int64) = vals[component, place]
get_component_value(vals::Vector, component::Int64, place::Int64) = get_component_value(vals[component], place)
...
get_component_values(vals::Matrix{Float64}, place::Int64, nComponents::Int64) = (@view vals[:, place])
get_component_values(vals, place::Int64, nComponents::Int64) = [get_component_value(vals, component, place) for component in 1:nComponents]

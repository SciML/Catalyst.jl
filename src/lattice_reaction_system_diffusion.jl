...
compartment_parameters(lrs::LatticeReactionSystem) = filter(p -> !is_spatial_param(p, lrs), parameters(lrs))
diffusion_parameters(lrs::LatticeReactionSystem) = filter(p -> is_spatial_param(p, lrs), parameters(lrs))

# Checks whenever a parameter is a spatial parameter or not. 
function is_spatial_param(p, lrs)
    hasmetadata(p, DiffusionParameter) && getmetadata(p, DiffusionParameter) && (return true)    # Wanted to just depend on metadata, but seems like we cannot implement that trivially.
...
split_parameters(ps::Vector{<:Number}, args...) = error("When providing parameters for a spatial system as a single vector, the paired form (e.g :D =>1.0) must be used.")
function split_parameters(ps::Vector{<:Pair}, p_comp_symbols::Vector, p_diff_symbols::Vector)
    pC_in = [p for p in ps if Symbol(p[1]) in p_comp_symbols]
    pD_in = [p for p in ps if Symbol(p[1]) in p_diff_symbols]
    (sum(length.([pC_in, pD_in])) != length(ps)) && error("These input parameters are not recongised: $(setdiff(first.(ps), vcat(first.([pC_in, pE_in]))))")
    return pC_in,pD_in
end
...
function lattice_process_input(input::Vector{<:Pair}, symbols::Vector{Symbol}, args...)
    (length(setdiff(Symbol.(first.(input)), symbols)) != 0) && error("Some input symbols are not recognised: $(setdiff(Symbol.(first.(input)), symbols)).")
    sorted_input = sort(input; by=p->findfirst(ModelingToolkit.getname(p[1]) .== symbols))
...
param_dict(pC, pD, lrs) = merge(vals_to_dict(compartment_parameters(lrs), pC), vals_to_dict(diffusion_parameters(lrs), pD))

# Computes the diffusion rates and stores them in a format (Dictionary of species index to rates across all edges).
function compute_all_diffusion_rates(pC::Vector{Vector{Float64}}, pD::Vector{Vector{Float64}}, lrs::LatticeReactionSystem)
    param_value_dict = param_dict(pC, pD, lrs)
    return [s => Symbolics.value.(compute_diffusion_rates(get_diffusion_rate_law(s, lrs), param_value_dict, lrs.nE)) for s in diffusion_species(lrs)]
end
function get_diffusion_rate_law(s::Symbolics.BasicSymbolic, lrs::LatticeReactionSystem)
    rates = filter(sr -> isequal(ModelingToolkit.getname(s),sr.species_sym), lrs.spatial_reactions)
...
function compute_diffusion_rates(rate_law::Num, param_value_dict::Dict{Any,Vector{Float64}}, nE::Int64)
    relevant_parameters = Symbolics.get_variables(rate_law)
    if all(length(param_value_dict[P])==1 for P in relevant_parameters) 
        return [substitute(rate_law, Dict(p => param_value_dict[p][1] for p in relevant_parameters))]
    end
    return [substitute(rate_law, Dict(p => get_component_value(param_value_dict[p], idxE) for p in relevant_parameters)) for idxE in 1:nE]    
end
...
function build_odefunction(lrs::LatticeReactionSystem, pC::Vector{Vector{Float64}}, pD::Vector{Vector{Float64}}, use_jac::Bool, sparse::Bool)
...
function build_f(ofunc::SciMLBase.AbstractODEFunction{true}, pC,
        diffusion_rates::Vector{Pair{Int64,Vector{Float64}}}, lrs::LatticeReactionSystem)
...
function build_jac(ofunc::SciMLBase.AbstractODEFunction{true}, pC,
    ...
    add_diff_J_vals = (sparse ? J -> (J.nzval .+= jac_prototype.nzval) : J -> (J .+= Matrix(jac_prototype)))
...
function build_jac_prototype(ns_jac_prototype::SparseMatrixCSC{Float64, Int64},
    ...
    only_diff = [(s in diff_species) && !Base.isstored(ns_jac_prototype, s, s) for s in 1:(lrs.nS)]
...
function JumpProcesses.JumpProblem(lrs::LatticeReactionSystem, dprob, aggregator, args...; name = nameof(lrs.rs), combinatoric_ratelaws = get_combinatoric_ratelaws(lrs.rs), checks = false, kwargs...)
    ...
    ___dprob = DiscreteProblem(reshape(dprob.u0, lrs.nS, lrs.nC), dprob.tspan, first.(dprob.p[1]))
    return JumpProblem(___dprob, aggregator, majumps, hopping_constants = hopping_constants, spatial_system = lrs.lattice, save_positions = (true, false))
end
...
function make_hopping_constants(dprob::DiscreteProblem, lrs::LatticeReactionSystem)
    diffusion_rates_dict = Dict(Catalyst.compute_all_diffusion_rates(dprob.p[1], dprob.p[2], lrs))
    all_diff_rates = [haskey(diffusion_rates_dict, s) ? diffusion_rates_dict[s] : [0.0] for s in species(lrs)]
    if length.(all_diff_rates) == 1
        return Catalyst.matrix_expand_component_values(all_diff_rates, length(vertices(lrs.lattice)))
    else
        ...
        for (e_idx, e) in enumerate(edges(lrs.lattice))
            for s_idx in 1:lrs.nS
                push!(hopping_constants[s_idx, e.src], Catalyst.get_component_value(all_diff_rates[s_idx], e_idx))
            end
        end
...
function make_majumps(dprob::DiscreteProblem, lrs::LatticeReactionSystem, aggregator, hopping_constants::Union{Matrix{<:Number}, Matrix{<:Vector}}, combinatoric_ratelaws, checks)
    any(length.(dprob.p[1]) .> 1) && error("Currently, for lattice jump simulations, spatial non-diffusion parameters are not supported.")
    ___dprob = DiscreteProblem(lrs.rs, reshape(dprob.u0, lrs.nS, lrs.nC), dprob.tspan, first.(dprob.p[1]))
    ___jprob = JumpProblem(lrs.rs, ___dprob, aggregator; hopping_constants=hopping_constants, spatial_system = lrs.lattice, combinatoric_ratelaws=combinatoric_ratelaws, checks=checks)
    (length(___jprob.variable_jumps) != 0)  && error("Currently, for lattice jump simulations, variable rate jumps are not supported.")
...
get_component_value(values::Vector{<:Vector}, component_idx::Int64, location_idx::Int64) = get_component_value(values[component_idx], location_idx)
get_component_value(values::Vector{<:Vector}, component_idx::Int64, location_idx::Int64, location_types::Vector{Bool}) = get_component_value(values[component_idx], location_idx, location_types[component_idx])
get_component_value(values::Vector{<:Number}, location_idx::Int64) = get_component_value(values, location_idx, length(values)==1)
get_component_value(values::Vector{<:Number}, location_idx::Int64, location_type::Bool) = location_type ? values[1] : values[location_idx]
# Converts a vector of vectors to a long vector.
expand_component_values(values::Vector{<:Vector}, n) = vcat([get_component_value.(values,comp) for comp in 1:n]...)
expand_component_values(values::Vector{<:Vector}, n, location_types::Vector{Bool}) = vcat([get_component_value.(values,comp,location_types) for comp in 1:n]...)
...
matrix_expand_component_values(values::Vector{<:Vector}, n) = reshape(expand_component_values(values, n), length(values), n)
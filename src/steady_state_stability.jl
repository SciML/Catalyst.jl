...
"""
    steady_state_stability(u::Vector, rs::ReactionSystem, p; tol = 10*sqrt(eps()),
...
- `tol = 10*sqrt(eps())`: The tolerance used for determining whether computed eigenvalues real
  parts can reliably be considered negative/positive. In practice, a steady state is considered
  stable if the corresponding Jacobian's maximum eigenvalue real part is < 0. However, if this
  maximum eigenvalue is in the range `-tol < eig < tol`, an error is thrown, as we do not deem
  that stability can be ensured with enough certainty. The choice `tol = 10*sqrt(eps())` has
  *not* been subject to much analysis.
- `ss_jac = steady_state_jac(rs; u0 = u)`: It is possible to pre-compute the Jacobian used
  for stability computation using `steady_state_jac`. If stability is computed for many states,
  precomputing the Jacobian may speed up evaluation.
...
- The input `u` can (currently) be both a vector of values (like `[1.0, 2.0]`) and a vector map
  ...
  produce vectors of values. However, if possible, it is recommended to work with values on the
  form of maps.
- Catalyst currently computes steady state stabilities using the naive approach of checking whether
  ...
  methods exist (and would be a welcome addition to Catalyst), there are no direct plans to
  implement these. Furthermore, Catalyst uses a tolerance `tol = 10*sqrt(eps())` to determine
...
a large number of stability computations has to be carried out in a performant manner.

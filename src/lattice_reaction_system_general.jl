...
struct SpatialReaction <: AbstractSpatialReaction
    ...
    function SpatialReaction(rate, substrates::Tuple{Vector, Vector},
...
struct SpatialReactionIndexed
    ...
    function SpatialReactionIndexed(sr::SpatialReaction, species_list::Vector{Symbol},
                                    param_list::Vector{Symbol}; reverse_direction = false)
        ...
        netstoich = Tuple([Pair.(get_s_idx.(first.(sr.netstoich[i])),
                                 last.(sr.netstoich[i])) for i in 1:2])
        netstoich_new = Tuple([Dict(Pair.(get_s_idx.(first.(sr.netstoich[i])),
                                          last.(sr.netstoich[i]))) for i in 1:2])
        ...
        new(rate, substrates, products, sr.substoich, sr.prodstoich, netstoich,
            netstoich_new, sr.only_use_rate)
    end
end
...
"""
function DiffusionReaction(rate, species)
    SpatialReaction(rate, ([species], Symbol[]), (Symbol[], [species]), ([1], Int64[]),
                    (Int64[], [1]))
end
...
"""
function OnewaySpatialReaction(rate, substrates::Vector, products::Vector,
                               substoich::Vector{Int64}, prodstoich::Vector{Int64})
    SpatialReaction(rate, (substrates, Symbol[]), (Symbol[], products),
                    (substoich, Int64[]), (Int64[], prodstoich))
end
...
struct LatticeReactionSystem # <: MT.AbstractTimeDependentSystem # Adding this part messes up show, disabling me from creating LRSs
    ...
    function LatticeReactionSystem(rs, spatial_reactions, lattice::DiGraph)
        return new(rs, spatial_reactions, unique(getfield.(spatial_reactions, :rate)),
                   lattice)
    end
...
function build_odefunction(lrs::LatticeReactionSystem, use_jac::Bool, sparse::Bool)
    ...
    spatial_reactions = [SpatialReactionIndexed(sr, map(s -> Symbol(s.f), species(lrs.rs)),
                                                lrs.spatial_params)
                         for sr in lrs.spatial_reactions]
    spatial_reactions_reversed = [SpatialReactionIndexed(sr,
                                                         map(s -> Symbol(s.f),
                                                             species(lrs.rs)),
                                                         lrs.spatial_params;
                                                         reverse_direction = true)
                                  for sr in lrs.spatial_reactions]
    spatial_reactions_dict = Dict{Int64, Vector{SpatialReactionIndexed}}([i => Vector{
                                                                                      SpatialReactionIndexed
                                                                                      }()
                                                                          for i in 1:nS])
    ...
    jac = (use_jac ?
           build_jac(ofunc, nS, nV, spatial_reactions, spatial_reactions_dict,
                     lrs.lattice.fadjlist) : nothing)
    jac_prototype = (use_jac ? build_jac_prototype(nS, nV, spatial_reactions, lrs, sparse) :
                     nothing)
...
function build_f(ofunc::SciMLBase.AbstractODEFunction{true}, nS::Int64, nV::Int64,
                 spatial_reactions::Vector{SpatialReactionIndexed},
                 adjlist::Vector{Vector{Int64}})
    return function (du, u, p, t)
        # Updates for non-spatial reactions.
        for comp_i::Int64 in 1:nV
            ofunc((@view du[get_indexes(comp_i, nV, nV * nS)]),
                  (@view u[get_indexes(comp_i, nV, nV * nS)]), (@view p[1][:, comp_i]), t)
        end

        # Updates for spatial reactions.
        for comp_i::Int64 in 1:nV
            for comp_j::Int64 in adjlist[comp_i],
                sr::SpatialReactionIndexed in spatial_reactions

                rate::Float64 = get_rate(sr, p[2],
                                         (@view u[get_indexes(comp_i, nV, nV * nS)]),
                                         (@view u[get_indexes(comp_j, nV, nV * nS)]))
...
function get_rate(sr::SpatialReactionIndexed, pE::Matrix{Float64}, u_src, u_dst)
    ...
    !isempty(sr.substrates[1]) &&
        for (sub::Int64, stoich::Int64) in zip(sr.substrates[1], sr.substoich[1])
            product *= u_src[sub]^stoich / factorial(stoich)
        end
    !isempty(sr.substrates[2]) &&
        for (sub::Int64, stoich::Int64) in zip(sr.substrates[2], sr.substoich[2])
            product *= u_dst[sub]^stoich / factorial(stoich)
        end
    return product
end

function build_jac(ofunc::SciMLBase.AbstractODEFunction{true}, nS::Int64, nV::Int64,
                   spatial_reactions::Vector{SpatialReactionIndexed},
                   spatial_reactions_dict::Dict{Int64, Vector{SpatialReactionIndexed}},
                   adjlist::Vector{Vector{Int64}})
    return function (J, u, p, t)
        ...
        # Updates for non-spatial reactions.
        for comp_i::Int64 in 1:nV
            ofunc.jac((@view J[get_indexes(comp_i, nV, nV * nS),
                               get_indexes(comp_i, nV, nV * nS)]),
                      (@view u[get_indexes(comp_i, nV, nV * nS)]), (@view p[1][:, comp_i]),
                      t)
        end
        ...
        for comp_i::Int64 in 1:nV
            for comp_j::Int64 in adjlist[comp_i],
                ...
                for sub::Int64 in sr.substrates[1]
                    rate::Float64 = get_rate_differential(sr, p[2], sub,
                                                          (@view u[get_indexes(comp_i, nV,
                                                                               nV * nS)]),
                                                          (@view u[get_indexes(comp_j, nV,
                                                                               nV * nS)]))
                    for stoich::Pair{Int64, Int64} in sr.netstoich[1]
                        J[get_index(comp_i, stoich[1], nV), get_index(comp_i, sub, nV)] += rate *
                                                                                           stoich[2]
                    end
                    for stoich::Pair{Int64, Int64} in sr.netstoich[2]
                        J[get_index(comp_j, stoich[1], nV), get_index(comp_i, sub, nV)] += rate *
                                                                                           stoich[2]
                    end
                end
                for sub::Int64 in sr.substrates[2]
                    rate::Float64 = get_rate_differential(sr, p[2], sub,
                                                          ...
                                                          (@view u[get_indexes(comp_i, nV,
                                                                               nV * nS)]))
                    for stoich::Pair{Int64, Int64} in sr.netstoich[1]
                        J[get_index(comp_i, stoich[1], nV), get_index(comp_j, sub, nV)] += rate *
                                                                                           stoich[2]
                    end
                    for stoich::Pair{Int64, Int64} in sr.netstoich[2]
                        J[get_index(comp_j, stoich[1], nV), get_index(comp_j, sub, nV)] += rate *
                                                                                           stoich[2]
                    end
                end
            end
...
function get_rate_differential(sr::SpatialReactionIndexed, pE::Matrix{Float64},
                               diff_species::Int64, u_src, u_dst)
    product::Float64 = pE[sr.rate]
    !isempty(sr.substrates[1]) &&
        for (sub::Int64, stoich::Int64) in zip(sr.substrates[1], sr.substoich[1])
            if diff_species == sub
...
function build_jac_prototype(nS::Int64, nV::Int64,
                             spatial_reactions::Vector{SpatialReactionIndexed},
                             lrs::LatticeReactionSystem, sparse::Bool)
    jac_prototype = (sparse ? spzeros(nS * nV, nS * nV) :
                     zeros(nS * nV, nS * nV)::Matrix{Float64})
    ...
    for comp_i::Int64 in 1:nV
        for comp_j::Int64 in lrs.lattice.fadjlist[comp_i]::Vector{Int64},
            sr::SpatialReactionIndexed in spatial_reactions

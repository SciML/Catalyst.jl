...
function make_compound(expr)
    ...
    ivs_get_expr = :(unique(reduce(
        vcat, (sorted_arguments(unwrap(comp))
    ...
    iv_check_expr = Expr(:escape,
        :(issetequal(arguments(unwrap($species_name)), $ivs_get_expr) ||
          error("The independent variable(S) provided to the compound ($(arguments(unwrap($species_name)))), and those of its components ($($ivs_get_expr)))), are not identical.")))

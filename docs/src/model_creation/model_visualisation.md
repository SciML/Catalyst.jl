...
```
!!! note
    Internally, `latexify(brusselator; form = :ode)` calls `latexify(make_rre_ode(brusselator))`. Hence, if you have already generated the `ODESystem` corresponding to your model, it can be used directly as input to `latexify`.

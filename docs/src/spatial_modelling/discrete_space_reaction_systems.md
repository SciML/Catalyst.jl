# [Introduction to Spatial Modelling with Catalyst](@id spatial_dspace_modelling_intro)
...
This tutorial introduces spatial modelling on discrete domains. It describes the basics of creating and simulating such models. To do so, it uses ODE simulations as examples. Additional tutorials provide further details on how to interact with [spatial simulation structures](@ref dspace_simulation_plotting) and [plot spatial simulations](@ref dspace_simulation_plotting), and also provide further details on [ODE](@ref spatial_dspace_ode_simulations) and [jump](@ref spatial_dspace_jump_simulations) simulations, respectively.


## [Basic example of a spatial simulation on a discrete domain](@id spatial_dspace_modelling_intro_example)
To perform discrete-space spatial simulations, the user must first define a [`DiscreteSpaceReactionSystem`](@ref). These combine:
...
- A space defining the spatial domain's compartments and how they are connected.

Here, as an example, we will simulate a spatial [two-state model](@ref basic_CRN_library_two_states). To do so, we first define our (non-spatial) model, the spatial reactions, and the space. These are then bundled into a [`DiscreteSpaceReactionSystem`](@ref).
...
More details on spatial reactions are available [here](@ref spatial_dspace_modelling_intro_spatial_reactions). In addition to Cartesian grid spaces (in 1, 2, and 3 dimensions), masked and unstructured (graph) spaces are also supported. The different discrete space types described in more detail [here](@ref spatial_dspace_modelling_intro_dspaces).

Once created, [`DiscreteSpaceReactionSystem`](@ref)s can be used as input to various problem types, which then can be simulated using the same syntax as non-spatial models. Here, we prepare an ODE simulation by creating an `ODEProblem`:
```@example spatial_intro_basics
...
nothing # hide
```
In this example we used non-uniform values for $X1$'s initial condition, but uniform values for the remaining initial condition and parameter values. More details of uniform and non-uniform initial conditions and parameter values are provided [here](@ref spatial_dspace_modelling_intro_simulation_inputs). We also note that the diffusion reaction introduces a new parameter, $D$ (determining $X1$'s diffusion rate), whose value must be designated in the parameter vector. 
...
nothing # hide
```
We note that simulations of spatial models are often computationally expensive. Advice on the performance of spatial ODE simulations is provided [here](@ref spatial_dspace_ode_simulations_solvers). 
...
```@example spatial_intro_basics
import CairoMakie
dspace_animation(sol, :X1, dsrs, "dspace_simulation.mp4")
```
![](./dspace_simulation.mp4)
More information on how to retrieve values from spatial simulations can be found [here](@ref dspace_simulation_structure_interaction_simulation_species), and for plotting them, [here](@ref dspace_simulation_plotting). Finally, a list of functions for querying `DiscreteSpaceReactionSystems` for various properties can be found [here](@ref api_dspace_simulations).

## [Spatial reactions](@id spatial_dspace_modelling_intro_spatial_reactions)
...
nothing # hide
```
Note that in this example, we specifically designate $D$ as an [edge parameter](@ref spatial_dspace_modelling_intro_simulation_edge_parameters).

## [Defining discrete spatial domains](@id spatial_dspace_modelling_intro_dspaces)
...
Catalyst supports three distinct types of discrete spaces:
- [Cartesian spaces](@ref spatial_dspace_modelling_intro_dspaces_cartesian). These are grids where each grid point corresponds to a compartment. Spatial transportation is permitted between adjacent compartments.
- [Masked spaces](@ref spatial_dspace_modelling_intro_dspaces_masked). In these grids, only a subset of the grid point actually corresponds to compartments. Spatial transportation is permitted between adjacent compartments.
- [Unstructured (or graph) spaces](@ref spatial_dspace_modelling_intro_dspaces_graph). These are defined by graphs, where vertices correspond to compartments and edges connect adjacent compartments.

Here, Cartesian spaces are a subset of the masked spaces, which are a subset of the unstructured spaces. If possible, it is advantageous to use as narrow a space definition as possible (this may both improve simulation performance and simplify syntax). Cartesian and masked spaces can be defined as one, two, and three-dimensional. By default, these spaces assume that diagonally neighbouring compartments are non-adjacent (do not permit direct movement of species in between themselves). To change this, provide the `diagonally_adjacent = true` argument to your [`DiscreteSpaceReactionSystem`](@ref) when it is created.

### [Defining Cartesian spaces](@id spatial_dspace_modelling_intro_dspaces_cartesian)
A Cartesian space is defined using the `CartesianGrid` function, which takes a single argument. For a 1d grid, simply provide the length of the grid as a single argument:
```@example spatial_intro_dspaces
using Catalyst # hide
...
### [Defining masked spaces](@id spatial_dspace_modelling_intro_dspaces_masked)
Masked spaces are defined through 1d, 2d, or 3d Boolean arrays. Each position in the array is `true` if it corresponds to a compartment, and `false` if it does not. E.g. to define a 1d grid corresponding to two disjoint sets, each with 3 compartments, use:
```@example spatial_intro_dspaces
...
Since graphs can represent any network of connected compartments, they do not have dimensions (like Cartesian or masked spaces). Another feature of graph spaces is that they can have non-symmetric connections, i.e. pairs of compartments where spatial movement of species is only permitted in one direction (in practice, this can be done for Cartesian and masked spaces as well, by [defining non-uniform spatial rates](@ref spatial_dspace_modelling_intro_simulation_inputs) and setting them to zero in one direction). This can be done by using a [*directed graph*](https://juliagraphs.org/Graphs.jl/dev/algorithms/digraph/) as input. E.g. here we define a directed cyclic graph, where movement is only allowed in one direction of the cycle:
```@example spatial_intro_dspaces
cycle_digraph(7)
nothing # hide
```

## [Non-uniform initial conditions and parameter values](@id spatial_dspace_modelling_intro_simulation_inputs)
For spatial models, initial conditions and parameter values are provided similarly as for non-spatial models. Wherever a single value is provided, it is used *uniformly* across the space. E.g. if we, for our [previous two-state model](@ref spatial_dspace_modelling_intro_example), set
```@example spatial_intro_nonuniform_vals
u0 = [:X1 => 1.0, :X2 => 2.0]
ps = [:k1 => 1.0, :k2 => 2.0, :D => 0.2]
nothing # hide
```
The initial condition will be $1.0$ for $X1$ across compartments, and $2.0$ for $X2$. Furthermore, for each simulation, in each compartment, the value of $1.0$ will be used for $k1$ and $2.0$ for $k2$. Finally, the transportation rate of $X1$ (set by the parameter $D$) will be $0.2$ across all connections. To set non-uniform values, non-scalar values must be provided in the map. How to do this depends on how the [space was defined](@ref spatial_dspace_modelling_intro_dspaces). Furthermore, some parameters that are part of spatial reactions may have their value tied to *connections between compartments*, rather than compartments (we call these *edge parameters*). These are handled slightly differently. How to designate parameters as either *edge parameters* or *compartment parameters* is described [here](@ref spatial_dspace_modelling_intro_simulation_edge_parameters).

Below we describe how to set non-uniform values in the various cases.

### [Non-uniform compartment values for Cartesian spaces](@id spatial_dspace_modelling_intro_simulation_inputs_cartesian)
To provide non-uniform values across a Cartesian space, simply provide the values in an array of the same dimension and size as the Cartesian space. E.g. for a 5x10 Cartesian space:
```@example spatial_intro_nonuniform_vals
using Catalyst # hide
ccart_dspaces = CartesianGrid((5, 10))
nothing # hide
```
...
### [Non-uniform compartment values for masked spaces](@id spatial_dspace_modelling_intro_simulation_inputs_masked)
Non-uniform values for masked spaces are provided in the same manner as for Cartesian spaces (however, values at coordinates that do not hold compartments are ignored). E.g. To provide random values for a masked space contained within a 5x10 Cartesian space we can again set:
```@example spatial_intro_nonuniform_vals
[:X1 => rand(5, 10), :X2 => 10.0]
nothing # hide
```
...
### [Non-uniform compartment values for unstructured spaces](@id spatial_dspace_modelling_intro_simulation_inputs_graphs)
...
### [Non-uniform values for edge-parameters](@id spatial_dspace_modelling_intro_simulation_inputs_edge_parameters)
Adjacent compartments are connected by edges (with which compartments are connected by edges being defined by the space). For unstructured spaces, it is possible (if a directed graph was used) to have edges from one compartment to another, but not in the opposite direction. For a space with $N$ compartments, edge values are set by a $NxN$ matrix, where value $(i,j)$ corresponds to the parameter's values in the edge going *from* compartment $i$ *to* compartment $j$. This matrix can be either [sparse or non-sparse](https://docs.julialang.org/en/v1/stdlib/SparseArrays/). In the latter cases, values corresponding to non-existing edges are ignored. 

Let's consider a 1d Cartesian space with 4 compartments. Here, an edge parameter's values are provided in a 4x4 matrix. For [the Brusselator model described previously](@ref spatial_dspace_modelling_intro_example), $D$'s value was tied to edges. If we wish to set the value of $D$ to various values between $0.1$ and $0.4$ we can do:
```@example spatial_intro_nonuniform_vals
ps = [:k1 => 1.0, :k2 => 2.0, 
      :D => [
        0.0 0.1 0.0 0.0;
...
## [Edge parameters and compartment parameters](@id spatial_dspace_modelling_intro_simulation_edge_parameters)
...
To learn the compartment and edge parameters of a `DiscreteSpaceReactionSystem`, the [`vertex_parameters`](@ref) and [`edge_parameters`](@ref) functions can be used:
```@example spatial_intro_edge_ps
two_state_model = @reaction_network begin
    (k1,k2), X1 <--> X2
end
...
dsrs = DiscreteSpaceReactionSystem(two_state_model, [diffusion_rx], space)
edge_parameters(dsrs)
```

## [Spatial modelling limitations](@id spatial_dspace_modelling_intro_limitations)
Many features which are supported for non-spatial `ReactionSystem`s are currently unsupported for [`DiscreteSpaceReactionSystem`](@ref)s. This includes [observables](@ref dsl_advanced_options_observables), [algebraic and differential equations](@ref coupled_models), [hierarchical models](@ref compositional_modeling), and [events](@ref events). It is possible that these features will be supported in the future. Furthermore, [removal of conserved quantities](@ref conservation_laws) is not supported when creating spatial `ODEProblem`s.

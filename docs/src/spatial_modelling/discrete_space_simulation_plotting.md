# [Plotting Spatial Simulations](@id dspace_simulation_plotting)
...
- [`dspace_plot`](@ref): Which plots a discrete space simulation at a specified time point.
- [`dspace_animation`](@ref): Which creates an animation of a discrete space simulation across the entire time series.
- [`dspace_kymograph`](@ref): Which, for a 1d Cartesian or masked discrete space based simulation, creates a kymograph of the simulation across time and space.

The first two functions can be applied to [graph](@ref spatial_dspace_modelling_intro_dspaces_graph) and 1d and 2d [Cartesian](@ref spatial_dspace_modelling_intro_dspaces_cartesian) and [masked](@ref spatial_dspace_modelling_intro_dspaces_masked) space based simulations, while `dspace_kymograph` can only be applied to 1d Cartesian and masked space-based simulations. Currently, there is no functionality for plotting simulations based on 3d Cartesian and masked spaces. Here we will demonstrate all plotting functions using ODE simulations, but they work equally well for jump simulations.
...
nothing # hide
```
To plot the simulation at a specific time point we use the `dspace_plot` function. In addition to the simulation we wish to plot, it takes the species we wish to plot and the [`DiscreteSpaceReactionSystem`](@ref) which generated the simulation as arguments. It also takes the time point at which we wish to plot the simulation as an additional argument (if not provided, the simulation will be plotted at the final time point). To use the [`dspace_plot`](@ref) function (or any other of Catalyst's spatial plotting functions) we also need to load the [CairoMakie](https://github.com/MakieOrg/Makie.jl) package (here, `import CairoMakie` is enough, and `using CairoMakie` is not required).
```@example dspace_plotting_1d
import CairoMakie
dspace_plot(sol, :X1, dsrs; t = 2.0)
```

If we instead wish to create an animation of our solution across the entire simulation, we can use the [`dspace_animation`](@ref) function. This takes a fourth required argument, a file to which the animation is saved.
```@example dspace_plotting_1d
dspace_animation(sol, :X1, dsrs, "dspace_simulation_1d.mp4")
```
![](./dspace_simulation_1d.mp4)
...
Finally, we can display a kymograph of our simulation across the full time span using [`dspace_kymograph`](@ref).
```@example dspace_plotting_1d
dspace_kymograph(sol, :X1, dsrs)
```
...
For more information of either function, and additional optional arguments, please read their corresponding api sections ([`dspace_plot`](@ref), [`dspace_animation`](@ref), and [`dspace_kymograph`](@ref)).
...
An animation of the solution can be created in a similar manner as for [the one-dimensional case](@ref dspace_simulation_plotting_1d_grids):
```@example dspace_plotting_2d
dspace_animation(sol, :X, dsrs, "dspace_simulation_2d.mp4")
```
![](./dspace_simulation_2d.mp4)

Again, please check the API pages for the [`dspace_plot`](@ref) and [`dspace_animation`](@ref) functions to see more details of their various options.
...
nothing # hide
```
As for Cartesian and masked space-based simulations we can plot this simulation using the `dspace_plot` function. In this case, however, we need to first load *both* [CairoMakie](https://github.com/MakieOrg/Makie.jl) and the [GraphMakie](https://github.com/MakieOrg/GraphMakie.jl) packages.
```@example dspace_plotting_graphs
import CairoMakie, GraphMakie
dspace_plot(osol, :X, dsrs)
```
Here we provide no `t` argument, and hence plot the solution at the final time point. As for 2d simulations, a colour scale described $X$'s concentration in each compartment. Some arguments, are not relevant when plotting simulations on Cartesian and masked spaces, but which are relevant to graph spaces are:
...
dspace_plot(osol, :X, dsrs; layout)
```

Finally, animations of graph space simulation work similarly to 2d ones, but accept the additional arguments relevant to plotting graphs.

## [Final notes](@id dspace_simulation_plotting_notes)

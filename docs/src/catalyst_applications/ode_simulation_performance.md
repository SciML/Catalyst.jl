...
We have previously described how to perform ODE simulations of *chemical reaction network* (CRN) models. These simulations are typically fast, and requires little additional considerations. However, when a model is simulated many times (e.g as a part of solving an inverse problem), or is very large, simulation runtimes may become noticeable. Here we will give some advice on how to improve performance for these case.

Generally, there are few good ways to, before a simulation, determine the best options. Hence, while we below will provide several options, if you face an application for which reducing runtime is critical (e.g. if you need to simulate the same ODE many times), it might be required to manually trial these various options to see which yields the best performance. It should be noted that the default options typically performs well, and its is primarily for large models where investigating these options is worthwhile. All ODE simulations of Catalyst models are performed using the OrdinaryDiffEq.jl package, [which documentation](https://docs.sciml.ai/DiffEqDocs/stable/) provides an extensive description of these and their options.

Generally, this small check list provides a quick guide for dealing with ODE performance:
1. For single simulations of small models, use [DifferentialEquation's default solver choice](@ref ode_simulation_performance_solvers) and do not worry further of the issue.
2. Determine whether your problem is [non-stiff nor stiff](@ref ode_simulation_performance_stiffness), and use the `Tsit5` solver for the former and `Rodas5P` for the latter.
...
plot(sol1)
``` 
We note that we get a warning, indicating that a instability was detected (the typical indication of a non-stiff solver being used for a stiff ODE). Furthermore, the resulting plot ends at $t≈10$, meaning that the simulation was not completed. Indeed, we can confirm this by checking the *return code* of the solution object:
```@example ode_simulation_performance_1
SciMLBase.successful_retcode(sol1)
```
Next, we instead try the `Rodas5P` solver, which is designed for stiff problems:
```@example ode_simulation_performance_1
sol2 = solve(oprob, RodasP5())
plot(sol2)
``` 
...
Generally, ODE solvers can be divided into [*explicit* and *implicit* solvers](https://en.wikipedia.org/wiki/Explicit_and_implicit_methods). Roughly, explicit solvers (which includes `Tsit5`) are better for non-stiff problems, with implicit solvers (like `Rodas5P`) being required for stiff problems. While we could use implicit solvers for all problems (to guarantee successful simulations irrespective of stiffness), these are typically less performant on non-stiff problems (as compared to the explicit solvers). An important property of implicit solvers is that they requires the *computation of a Jacobian* as part of their routine. This means that the various options for efficient Jacobian computation [described later in this tutorial](@ref ode_simulation_performance_jacobian) are only relevant to implicit solvers.

Finally, we should note that this is not an exact science, and sometimes explicit solvers can successfully solve an stiff problem. E.g. if we change the parameters values of our previous Brusselator model to `[:A => 1.0, :B => 4.0]`, the `Tsit5` will successfully be able to simulate it.
...
oprob = ODEProblem(rn, u0, tspan, p; jac=true)
nothing # hide
```

#### Using a sparse Jacobian
For a system with $n$ variables, the Jacobian will be an $n\times n$ matrix. This means that, as $n$ becomes large, the Jacobian can become *very* large, potentially causing a significant strain on memory. In these cases, most Jacobian entries are typically $0$. In these cases it can often be advantageous to use a [*sparse*](https://en.wikipedia.org/wiki/Sparse_matrix) Jacobian (rather than a *dense* one, which is default). To designate sparse Jacobian usage, simply provide the `sparse=true` option when constructing an `ODEProblem`:
```@example ode_simulation_performance_2
oprob = ODEProblem(rn, u0, tspan, p; sparse=true)
nothing # hide
```

#### [Linear solver selection](@id ode_simulation_performance_symbolic_jacobian_linear_solver)
When implicit solvers uses e.g. the Newton-Raphson method to, in each simulation time step, solve a (typically non-linear) equation, they actually solve a linearised version of this equation. For this they use a linear solver, the choice of which can impact performance. To specify one, we use the `linsolve` option (given to the solver function, *not* the `solve` command). E.g. to use the `KLUFactorization` linear solver we run:
```@example ode_simulation_performance_2
solve(oprob, Rodas5P(linsolve = KLUFactorization()))
nothing # hide
```
A full list of potential linear solver can be found [here](https://docs.sciml.ai/LinearSolve/dev/solvers/solvers/#Full-List-of-Methods), however, the default choice selected by OrdinaryDiffEq typically performs well. 

A unique approach to the linear solvers is to use a Jacobian-free Newton-Krylov method. These does not actually computes the Jacobian, but rather *the effect of multiplying it with a vector*. They are typically advantageous for large systems (with large Jacobians), and can be designated using the `KrylovJL_GMRES` linear solver:
```@example ode_simulation_performance_2
solve(oprob, Rodas5P(linsolve = KrylovJL_GMRES()))
nothing # hide
```
Since these method does not depend on a Jacobian, certain Jacobian options (such as [computing it symbolically](@ref ode_simulation_performance_symbolic_jacobian)) are irrelevant to them. 

#### [Designating preconditioners for Jacobian-free linear solvers](@ref ode_simulation_performance_preconditioners)
When an implicit methods solves a linear equation through an iterative method, the rate of convergence depend on the numerical properties of the matrix defining the linear system. To speed up convergence, a [*preconditioner*](https://en.wikipedia.org/wiki/Preconditioner) can be applied to both sides of the linear equation, attempting to create a equation that converges faster. In practise, preconditioners are implemented as function with a specific set of arguments. How to implement these is non-trivial, and we recommend reading OrdinaryDiffEq's documentation pages [here](https://docs.sciml.ai/DiffEqDocs/stable/features/linear_nonlinear/#Preconditioners:-precs-Specification) and [here](https://docs.sciml.ai/DiffEqDocs/stable/tutorials/advanced_ode_example/#Adding-a-Preconditioner). In this example, we will define an [Incomplete LU](https://en.wikipedia.org/wiki/Incomplete_LU_factorization) preconditioner (which requires the IncompleteLU.jl package):
``` ode_simulation_performance_2
using IncompleteLU
...
plot(sol)
```
Due to degradation of $S$, if the conversion rate is not high enough, the total amount of $P$ produced is reduced. For these tutorial we will investigate this effect for a range of values of $kP$. This will require a large number of simulations (for various $kP$ values), which we will parallelise on CPUs (here) and GPUs ([later](@ref ode_simulation_performance_parallelisation_GPU)).

To parallelise our model simulations, we first need to create an `EnsembleProblem`, which will describe which simulations we wish to perform. The input to this is:
...
- `repeat`: The repeat of the specific simulations. We will ignore this one for now, but more details (and how it is different from `i`) is provided [here](https://docs.sciml.ai/DiffEqDocs/stable/features/ensemble/#Building-a-Problem).

and output the `ODEProblem` simulated in the ith simulation.

Let's assume that we wish to simulate our model 100 times, for $kP = 0.01, 0.02, ..., 0.99, 1.0$. We can now define our `prob_func` as:
```@example ode_simulation_performance_3
...
nothing # hide
```
Next, we can create our `EnsembleProblem`. These requires first loading the DifferentialEquations package (which includes all differential equations related functionality, including OrdinaryDiffEq):
```@example ode_simulation_performance_3
using DifferentialEquations
eprob = EnsembleProblem(oprob, prob_func)
nothing # hide
```
We can now solve our `ODEProblem` using the same syntax we would use solve the original `ODEProblem`, with the exception that an additional argument, `trajectories`, is required (which denotes how many simulations should be performed).
```@example ode_simulation_performance_3
...
```@example ode_simulation_performance_3
plot(0.01:0.01:1.0, map(sol -> sol[:P][end], esol.u), xguide="kP", yguide="P produced")
```

Above, we have simply used `EnsembleProblem` as an simple interface to run a large number of similar simulations. However, these problems have the advantage that they allow the passing of an *ensemble algorithm* to the `solve` command, which describe a strategy for parallelising the simulation. By default, `EnsembleThreads` is used. This parallelises the simulations using [multithreading](https://en.wikipedia.org/wiki/Multithreading_(computer_architecture)) (parallelisation within a single process, i.e. CPU), which is typically advantageous for small problems. An alternative is `EnsembleDistributed` which instead parallelises the simulations using [multiprocessing](https://en.wikipedia.org/wiki/Multiprocessing) (parallelisation across multiple processes). To do this, we simply supply this additional solver to the solve command:
```@example ode_simulation_performance_3
...
nothing # hide
```
Especially powerful personal computers and HPC clusters typically have a large number of available additional processes that can be added to improve performance.
...
Generally, we can parallelise `EnsembleProblem`s across several GPUs in a very similar manner to how we parallelised them across several CPUs, but by using a different ensemble algorithm (such as `EnsembleGPUKernel`). However, there are some additional caveats:
- GPU parallelisation requires loading and installing the [DiffEqGPU.jl](https://github.com/SciML/DiffEqGPU.jl) package.
- By default, Julia's decimal numbers are implemented as `Float64`s, however, using `Float32`s is advantageous on GPUs. Hence we specify all our initial condition and parameter values as such.
- Depending on which GPU hardware is used (e.g. NVIDIA's CUDA or Apple's Metal), a specific back-end package have to be installed, loaded, and activated.
- We have to designate an appropriate ensemble algorithm for GPU parallelisation.
- We much use a specific ODE solver specialised for computations on GPUs.
- We should designate all our vector's as [static vectors](https://github.com/JuliaArrays/StaticArrays.jl) using the `@SVector` macro.

Let us rephrase our `EnsembleProblem` using these constraints. We will assume that we are using the CUDA GPU harware, so we will first load the [CUDA.jl](https://github.com/JuliaGPU/CUDA.jl) package, as well as DiffEqGPU:

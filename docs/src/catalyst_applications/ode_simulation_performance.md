...
Generally, there are few good ways to, before a simulation, determine the best options. Hence, while we below will provide several options, if you face an application for which reducing runtime is critical (e.g. if you need to simulate the same ODE many times), it might be required to manually trial these various options to see which yields the best performance. It should be noted that the default options typically perform well, and it is primarily for large models where investigating these options is worthwhile. All ODE simulations of Catalyst models are performed using the OrdinaryDiffEq.jl package, [which documentation](https://docs.sciml.ai/DiffEqDocs/stable/) provides an extensive description of these and their options.

Generally, this small checklist provides a quick guide for dealing with ODE performance:
1. For single simulations of small models, use [DifferentialEquation's default solver choice](@ref ode_simulation_performance_solvers) and do not worry further about the issue.
...
For a system with $n$ variables, the Jacobian will be an $n\times n$ matrix. This means that, as $n$ becomes large, the Jacobian can become *very* large, potentially causing a significant strain on memory. In these cases, most Jacobian entries are typically $0$. In these cases, it can often be advantageous to use a [*sparse*](https://en.wikipedia.org/wiki/Sparse_matrix) Jacobian (rather than a *dense* one, which is the default). To designate sparse Jacobian usage, simply provide the `sparse=true` option when constructing an `ODEProblem`:
```@example ode_simulation_performance_3
...
nothing # hide
```
Finally, we note that since matrix-free linear solvers (like `KrylovJL_GMRES`) by default do not build a Jacobian, if we want to use them with a preconditioner we must tell them to build it. This can be done using the `concrete_jacobian=true` option:
```@example ode_simulation_performance_3
...
nothing # hide
```
Next, we can create our `EnsembleProblem`. These require first loading the DifferentialEquations package (which includes all differential equations related functionality, including OrdinaryDiffEq):
```@example ode_simulation_performance_4
using DifferentialEquations
...
- By default, Julia's decimal numbers are implemented as `Float64`s, however, using `Float32`s is advantageous on GPUs. Hence we specify all our initial conditions and parameter values as such.
- Depending on which GPU hardware is used (e.g. NVIDIA's CUDA or Apple's Metal), a specific back-end package has to be installed, loaded, and activated.
- We have to designate an appropriate ensemble algorithm for GPU parallelisation.
- We must use a specific ODE solver specialised for computations on GPUs.
- We should designate all our vectors as [static vectors](https://github.com/JuliaArrays/StaticArrays.jl) using the `@SVector` macro.

Let us rephrase our `EnsembleProblem` using these constraints. We will assume that we are using the CUDA GPU hardware, so we will first load the [CUDA.jl](https://github.com/JuliaGPU/CUDA.jl) package, as well as DiffEqGPU:
```@example ode_simulation_performance_4

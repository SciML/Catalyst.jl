...
rn = @reaction_network name begin k, A --> 0 end k
rntest(rn, :name)
...
rn = @reaction_network begin k, A --> 0 end k
rntest(rn, nameof(rn))

function makern(; name)
    @reaction_network $name begin k, A --> 0 end k
end
...
rn = @reaction_network begin π * k * D * hill(B, k2, B * D * H, n), 3 * A --> 2 * C end k k2 n
@parameters k, k2, n
...
AA = A
AAA = A^2 + B
rn = @reaction_network rn begin k * $AAA, C --> D end k
rn2 = ReactionSystem([Reaction(k * AAA, [C], [D])], t; name = :rn)
@test rn == rn2

rn = @reaction_network rn begin k, $AA + C --> D end k
rn2 = ReactionSystem([Reaction(k, [AA, C], [D])], t; name = :rn)
@test rn == rn2

BB = B;
A2 = A;
rn = @reaction_network rn begin (k1, k2), C + $A2 + $BB + $A2 <--> $BB + $BB end k1 k2
...
kk1 = k^2 * A
kk2 = k1 + k2
rn = @reaction_network rn begin α + $kk1 * $kk2 * $AA, 2 * $AA + B --> $AA end α
@parameters α
rn2 = ReactionSystem([Reaction(α + kk1 * kk2 * AA, [A, B], [A], [2, 1], [1])], t;
                     name = :rn)
@test rn == rn2

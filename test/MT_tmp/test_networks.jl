...
reaction_networks_standard[1] = @MT_reaction_network begin
    (p1,p2,p3), ∅ → (X1,X2,X3)
    (k1,k2), X2 ⟷ X1 + 2X3
    (k3,k4), X1 ⟷ X3
    (d1,d2,d3), (X1,X2,X3) → ∅
...
reaction_networks_standard[2] = @MT_reaction_network begin
    mmR(X2,v1,K1), ∅ → X1
    mm(X1,v2,K2), ∅ → X2
    d, X1+X2 → ∅
end v1 K1 v2 K2 d

reaction_networks_standard[3] = @MT_reaction_network begin
    mm(X2,v1,K1), ∅ → X1
    mm(X3,v2,K2), ∅ → X2
    (k1,k2), X1 ⟷ X4
    (k3,k4), X4 + X2 ⟷ X3 +X1
...
reaction_networks_standard[4] = @MT_reaction_network begin
    mmR(X4,v1,K1), ∅ → X1
    mmR(X1,v2,K2), ∅ → X2
    mmR(X2,v3,K3), ∅ → X3
    mmR(X3,v4,K4), ∅ → X4
...
reaction_networks_standard[5] = @MT_reaction_network begin
    p, ∅ → X1
    (k1,k2), X1 ⟷ X2
    (k3,k4), X2 ⟷ X3
    (k5,k6), X3 ⟷ X4
...
reaction_networks_standard[6] = @MT_reaction_network begin
    (p1,p2), ∅ → (X1,X2)
    (k1,k2), 2X1 ⟷ X3
    (k3,k4), X2 + X3 ⟷ 4X4
    (k5,k6), X4 + X1 ⟷ 2X3
...
reaction_networks_standard[7] = @MT_reaction_network begin
    (p1,p2,p3), ∅ → (X1,X2,X3)
    (k1,k2), X1 + 2X2 ⟷ X4
...
reaction_networks_standard[8] = @MT_reaction_network begin
    p, ∅ → 2X1
    k1, X1 → X2
    (k2, k3), X2 → X3
    d, X3 → ∅
...
reaction_networks_standard[9] = @MT_reaction_network begin
    (p1,p2,p3), ∅ ⟶ (X1,X2,X3)
    (d1,d2,d3), (X1,X2,X3) ⟶ ∅
    (k1,k2), X1 + X2 ⟷ X3
    (k3,k4), X3 ⟷ X4
...
reaction_networks_standard[10] = @MT_reaction_network begin
    p, ∅ ⟶ X1
    (k1,k2), X1 → X2
    (k3,k4), X2 → X3
    (k5,k6), X3 → X4
...
reaction_networks_hill[1] = @MT_reaction_network begin
    hillR(X2,v1,K1,n1),   ∅ → X1
    hillR(X1,v2,K2,n2),   ∅ → X2
    (d1,d2),          (X1,X2) → ∅
end v1 v2 K1 K2 n1 n2 d1 d2

reaction_networks_hill[2] = @MT_reaction_network begin
...
reaction_networks_hill[3] = @MT_reaction_network begin
    hillR(X2,v1,K1,n1), ∅ → X1
    hill(X1,v2,K2,n2), ∅ → X2
    d, X1+X2 → ∅
end v1 K1 n1 v2 K2 n2 d

reaction_networks_hill[4] = @MT_reaction_network begin
...
reaction_networks_hill[5] = @MT_reaction_network begin
...
reaction_networks_hill[6] = @MT_reaction_network begin
    v/10+hill(X1,v,K,n), ∅ → X1
    d, X1 → ∅
end v K n d

reaction_networks_hill[7] = @MT_reaction_network begin
...
reaction_networks_hill[8] = @MT_reaction_network begin
    hill(X2,v1,K1,n1), ∅ → X1
...
reaction_networks_hill[9] = @MT_reaction_network begin
    hill(X1,v1,K1,n1)*hillR(X1,v2,K2,n2), ∅ → X1
    d, X1 → ∅
end v1 K1 n1 v2 K2 n2 d

reaction_networks_hill[10] = @MT_reaction_network begin
    hill(X2,v1,K1,n1), ∅ → X1
...
reaction_networks_constraint_1 = @MT_reaction_network begin
    (k1,k2), X1 ↔ X2
    (k3,k4), X2 ↔ X3
    (k5,k6), X3 ↔ X1
...
reaction_networks_constraint_2 = @MT_reaction_network begin
    (k1,k2), X1 ↔ 2X1
    (k3,k4), X1 + X2 ↔ X3
    (k5,k6), X3 ↔ X2
end k1 k2 k3 k4 k5 k6 FC
...
reaction_networks_constraint_3 = @MT_reaction_network begin
    (k1,k2*X5), X1 ↔ X2
    (k3*X5,k4), X3 ↔ X4
    (p+k5*X2*X3,d), ∅ ↔ X5
...
reaction_networks_constraint_4 = @MT_reaction_network begin
    (k1,k2), X1 + X2 ↔ X3
    (mm(X3,v,K),d), ∅ ↔ X4
...
reaction_networks_constraint_5 = @MT_reaction_network begin
    (k1,k2), X1 ↔ 2X2
    (k3,k4), 2X2 ↔ 3X3
    (k5,k6), 3X3 ↔ 4X4
end k1 k2 k3 k4 k5 k6 FC
...
reaction_networks_constraint_6 = @MT_reaction_network begin
    mmR(X1,v1,K1), X1 → X2
    mmR(X2,v2,K2), X2 → X3
    mmR(X3,v3,K3), X3 → X1
...
reaction_networks_constraint_7 = @MT_reaction_network begin
    (k1,k2), X1 + X2 ↔ X3
    (mm(X3,v,K),d), ∅ ↔ X2
    (k3,k4), X2 ↔ X4
...
reaction_networks_constraint_8 = @MT_reaction_network begin
    (k1,k2), X1 + X2 ↔ X3
...
reaction_networks_constraint_9 = @MT_reaction_network begin
    (k1,k2), X1 + X2 ↔ X3
    (k3,k4), X3 + X4 ↔ X5
    (k5,k6), X5 + X6 ↔ X7
...
reaction_networks_constraint_10 = @MT_reaction_network rnType begin
...
# Brusselator.
reaction_networks_real[1] = @MT_reaction_network begin
    A, ∅ → X
    1, 2X + Y → 3X
    B, X → Y
    1, X → ∅
...
reaction_networks_real[2] = @MT_reaction_network begin
    v0 + hill(σ,v,K,n), ∅ → (σ+A)
    deg, (σ,A,Aσ) → ∅
    (kB,kD), A + σ ↔ Aσ
    S*kC, Aσ → σ
...
# A cell cycle model
cc_network = @MT_reaction_network begin
  k1, 0 --> Y
  k2p, Y --> 0
  k2pp*P, Y --> 0
...
# A bistable switch
reaction_networks_real[4] = @MT_reaction_network begin
    d,    (X,Y) → ∅
    hillR(Y,v1,K1,n1), ∅ → X
    hillR(X,v2,K2,n2), ∅ → Y
...
reaction_networks_weird[1] = @MT_reaction_network begin
    exp(p), ∅ → X
    exp(X), X → ∅
end p d

reaction_networks_weird[2] = @MT_reaction_network begin
    k1, ∅ → X
    k2*log(12+X), X → Y
    k3*ln(3+Y), Y → Z
    logk(5,6+k4), Z → ∅
...
reaction_networks_weird[3] = @MT_reaction_network begin
    d,    (X,Y) → ∅
    hillR(hill(Y,v2,K2,n2),v1,K1,n1), ∅ → X
    hillR(hill(X,v1,K1,n1),v2,K2,n2), ∅ → Y
end d v1 K1 n1 v2 K2 n2

reaction_networks_weird[4] = @MT_reaction_network begin
    p, ∅ → X1
    (k1,X2^X3), X1 ⟷ X2
    (X2/X5,k4), X2 ⟷ X3
    (k5,X1*X2*X4), X3 ⟷ X4
...
reaction_networks_weird[5] = @MT_reaction_network begin
    (k1*tanh(X1),k2), X1 ↔ 2X1
    (2+sin(X1),3+sin(k31)), X1 + X2 ↔ X3
    (k5,4+cos(X3+X1)+sin(X2)), X3 ↔ X2
end k1 k2 k3 k4 k5 k6

reaction_networks_weird[6] = @MT_reaction_network begin
    (p,d), ∅ ↔ X1
    (p,d), ∅ ↔ X1
    (p,d), ∅ ↔ X1
    (p,d), ∅ ↔ X1
...
reaction_networks_weird[7] = @MT_reaction_network begin
    k1, X1 → X2
    0, X2 → X3
    k2, X3 → X4
    k3, X4 → X5
...
reaction_networks_weird[8] = @MT_reaction_network begin
    k1+X3, X1 → X2
    k2*X4, X2 → X3
    k3/X5, X3 → X4
    X6/k4, X4 → X5
...
reaction_networks_weird[9] = @MT_reaction_network begin
    p, ∅ → X1
    hill(hill(hill(hill(X1,v1,K1,n1),v2,K2,n2),v3,K3,n3),v4,K4,n4), X1 → ∅
end p v1 K1 n1 v2 K2 n2 v3 K3 n3 v4 K4 n4

reaction_networks_weird[10] = @MT_reaction_network begin
    d, 5X1 → 4X1
end d

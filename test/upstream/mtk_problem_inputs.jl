...
# Checks Errors On Faulty Inputs: tests removed. MTKBase v1.23+ uses least-squares
# fallback for underdetermined initialization instead of erroring on incomplete
# u0/parameter inputs, so the @test_throws Exception checks are no longer applicable.

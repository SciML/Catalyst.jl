...
let
    ...
    @test maximum(abs.(f_eval(rs, hc_ss, ps, 0.0))) ≈ 0.0 atol = 1.0e-12
...
let
    ...
    @test length(sss) == 4
    for ss in sss
        @test ps[:v] * (0.1 / ps[:v] + ss[1]^ps[:n] / (ss[1]^ps[:n] + ps[:K]^ps[:n])) - ps[:d] * ss[1] ≈ 0.0 atol = 1.0e-12
    end
...
let
    ...
    @test allunique(sss)
    for ss in sss
        @test f_eval(rs, Pair.(unknowns(rs), ss), ps, 0.0) ≈ [0.0, 0.0] atol = 1.0e-12 rtol = 1.0e-12
    end
end

...
network1 = @min_reaction_network rn begin
    hill(X,4,3,2), X + Y --> Z1
    mm(X,4.0,3), X + Y --> Z2
    3X^2+1, X + Y --> Z3
    exp(Y), X + Y --> Z4
...
addsdes!(network1)
addjumps!(network1)
network2 = @min_reaction_network rn begin
...
model1 = @min_reaction_network rn begin
    (5,5000), X ↔ 0
end
addsdes!(model1)
model2 = @min_reaction_network rn η begin
    (5,5000), X ↔ 0
end
addsdes!(model2)
model3 = @min_reaction_network rn η begin
    (1,5000), X ↔ 0
end
addsdes!(model3)
...
sol1 = StochasticDiffEq.solve(prob1, ImplicitEM())
sol2 = StochasticDiffEq.solve(prob2, ImplicitEM())
sol3 = StochasticDiffEq.solve(prob3, ImplicitEM())

...
let
    ...
    for factor in [1.0e-2, 1.0e-1, 1.0e0, 1.0e1, 1.0e2]
        ...
        sol = solve(prob, Vern7(), saveat = t_stops, abstol = 1.0e-10, reltol = 1.0e-10)
...
let
    ...
    for factor in [1.0e-1, 1.0e0, 1.0e1]
...
let
    ...
    for (rn_catalyst, rn_manual, u0_sym, ps_sym) in zip(catalyst_networks, manual_networks, u0_syms, ps_syms)
        for factor in [1.0e-2, 1.0e-1, 1.0e0, 1.0e1]
            # Set input values.
...
let
    ...
    for factor in [1.0e0, 1.0e1, 1.0e2]

...
let
    ...
    for factor in [1.0e-2, 1.0e-1, 1.0e0, 1.0e1, 1.0e2], repeat in 1:10
        ...
        real_jac = [
            -1 - 2 * p[p3] * u[Y] * u[X] -p[p3] * u[X] * u[X] 1.0;
...
let
    ...
    for factor in [1.0e-2, 1.0e-1, 1.0e0, 1.0e1, 1.0e2], repeat in 1:10
        ...
        real_jac = [
            -2 * p[k1] * u[A] - p[k3] * u[B] 2 * p[k2] - p[k3] * u[A] p[k4] + 3 * p[k5] * u[C]^2 / 2;
...
let
    ...
    for factor in [0.1, 1.0, 10.0]
        ...
        @test eval_jac(oprob_sjac, true) ≈ eval_jac(sprob_sjac, true) atol = 1.0e-14 rtol = 1.0e-14
        @test eval_jac(oprob_sjac, true) ≈ eval_jac(nlprob_sjac, true) atol = 1.0e-14 rtol = 1.0e-14
    end
end
...
let
    ...
    for factor in [0.1, 1.0, 10.0]
        ...
        # Approx is due to https://github.com/SciML/ModelingToolkit.jl/issues/3554.
        @test Matrix(jac_sparse) ≈ jac atol = 1.0e-14 rtol = 1.0e-14
...
let
    ...
    sol = solve(oprob, Rosenbrock23(), saveat = 0.1, abstol = 1.0e-8, reltol = 1.0e-8)
    sol_j = solve(oprob_j, Rosenbrock23(), saveat = 0.1, abstol = 1.0e-8, reltol = 1.0e-8)
    sol_s = solve(oprob_s, Rosenbrock23(), saveat = 0.1, abstol = 1.0e-8, reltol = 1.0e-8)
    sol_js = solve(oprob_js, Rosenbrock23(), saveat = 0.1, abstol = 1.0e-8, reltol = 1.0e-8)

...
for factor in [1e-1, 1e0, 1e1, 1e2, 1e3]
    ...
    @test abs.((sol.u[end][6]^2 / factorial(2)) / (sol.u[end][7]^3 / factorial(3)) - p[6] / p[5]) <
          1e-8
...
function real_functions_1(du, u, p, t)
    ...
    return du[3] = p3 + 2 * k1 * X2 - 2 * k2 * X1 * X3^2 / factorial(2) + k3 * X1 - k4 * X3 -
                   d3 * X3
end
...
function real_functions_2(du, u, p, t)
    ...
    return du[2] = v2 * X1 / (K2 + X1) - d * X1 * X2
end
...
function real_functions_3(du, u, p, t)
    ...
    return du[3] = v3 * K3^n3 / (K3^n3 + X2^n3) - d3 * X3
end
...
function real_functions_4(du, u, p, t)
    ...
    return du[3] = -k5 * X3 + k6 * X1 + k3 * X2 - k4 * X3
end
...
function real_functions_5(du, u, p, t)
    ...
    return du[3] = k3 * log(3 + Y) * Y - log(5, 6 + k4) * Z
end

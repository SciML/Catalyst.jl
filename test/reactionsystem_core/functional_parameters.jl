...
let
    ...
    sol_base = solve(oprob_base; saveat = 0.1, abstol = 1.0e-8, reltol = 1.0e-8)
    sol_prog = solve(oprob_prog; saveat = 0.1, abstol = 1.0e-8, reltol = 1.0e-8)
    sol_dsl = solve(oprob_dsl; saveat = 0.1, abstol = 1.0e-8, reltol = 1.0e-8)

    # Checks that simulations are identical.
    @test sol_base[X] ≈ sol_prog[X] atol = 1.0e-8 rtol = 1.0e-8
    @test sol_base[X] ≈ sol_dsl[X] atol = 1.0e-8 rtol = 1.0e-8
end
...
let
    ...
    ode_sol_base = solve(ODEProblem(rs_base, u0, tend, ps_base); reltol = 1.0e-8, abstol = 1.0e-8, saveat = 0.1)
    ode_sol_pIn = solve(ODEProblem(rs_pIn, u0, tend, ps); reltol = 1.0e-8, abstol = 1.0e-8, saveat = 0.1)
    ...
    @test sde_sol_mean_base ≈ sde_sol_mean_pIn rtol = 1.0e-1 atol = 1.0e-1
    ...
    @test_broken jmp_sol_mean_base ≈ jmp_sol_meanpIn rtol = 1.0e-1 atol = 1.0e-1
end
...
let
    ...
    @test sol.u ≈ sol_funcp.u atol = 1.0e-6 rtol = 1.0e-6
end

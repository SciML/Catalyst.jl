using DiffEqBiological, OrdinaryDiffEq, Statistics, Test
...
ho_model1 = @min_reaction_network rn begin
    (1.0,1.0), X + 2X ↔ Y + X
end
addodes!(ho_model1)
addjumps!(ho_model1)
ho_model2 = @min_reaction_network rn begin
    (X^3/6,Y * X), X + 2X ⟺ Y + X
end
addodes!(ho_model2)
addjumps!(ho_model2)
ho_model3 = @min_reaction_network rn begin
...
sol_det1 = solve(prob_det1,Tsit5(), reltol=1e-8, abstol=1e-8)
sol_det2 = solve(prob_det2,Tsit5(), reltol=1e-8, abstol=1e-8)
# @test tmp_sol2vec(sol_det1,1) == tmp_sol2vec(sol_det2,1)
# @test tmp_sol2vec(sol_det1,2) == tmp_sol2vec(sol_det2,2)
@test isapprox(tmp_sol2vec(sol_det1,1), tmp_sol2vec(sol_det2,1), atol=1e-5)
@test isapprox(tmp_sol2vec(sol_det1,2), tmp_sol2vec(sol_det2,2), atol=1e-5)

...
function dspace_plot(
        sol, sp, dsrs::DiscreteSpaceReactionSystem{Q, R, <:AbstractGraph, T};
...
function dspace_animation(
        ...
        colormap = :BuGn_7, node_size = 50, ttitle::Bool = true, kwargs...
    ) where {Q, R, T}
    # Prepares the inputs to the figure.
    plot_graph = SimpleGraph(Catalyst.dspace(dsrs))
    ...
    fig, ax, plt = graphplot(
        plot_graph; node_color = Makie.@lift(vals[$frame]),

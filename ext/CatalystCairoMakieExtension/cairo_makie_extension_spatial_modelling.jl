...
function lattice_plot(sol, sp, lrs::LatticeReactionSystem{Q, R, <: Catalyst.GridLattice{1, S}, T};
        t = sol.t[end], markersize = 20, kwargs...) where {Q, R, S, T}
    vals = lat_getu(sol, sp, lrs; t = [t])
    vals = Catalyst.demask_vals(vals, lrs)[1]
    return scatterlines(vals; axis = (xlabel = "Compartment", ylabel = "$(sp)"), markersize = markersize, kwargs...)
end

# Internal dispatch for the animation of a lattice simulation on a 1d lattice (Cartesian or masked). 
function lattice_animation(sol, sp, lrs::LatticeReactionSystem{Q, R, <: Catalyst.GridLattice{1, S}, T}, filename::String;
...
function lattice_kymograph(sol, sp, lrs::LatticeReactionSystem{Q, R, <: Catalyst.GridLattice{1, S}, T}; colormap = :BuGn_7,
    ...
    return heatmap(t, y_vals, vals; axis = (xlabel = "Time", ylabel = "Compartment", xgridvisible = false, ygridvisible = false), colormap, colorrange = (plot_min, plot_max))
end
...
function lattice_plot(sol, sp, lrs::LatticeReactionSystem{Q, R, <: Catalyst.GridLattice{2, S}, T}; t = sol.t[end],
        colormap = :BuGn_7, plot_min = nothing, plot_max = nothing, kwargs...) where {Q, R, S, T}
    ...
    # Creates the figure.
    return heatmap(x_vals, y_vals, vals; axis = (xlabel = "Time", ylabel = "Compartment", xgridvisible = false, ygridvisible = false), colormap, colorrange = (plot_min, plot_max), kwargs...)
end

# Internal dispatch for the animation of a lattice simulation on a 2d lattice (Cartesian or masked). 
function lattice_animation(sol, sp, lrs::LatticeReactionSystem{Q, R, <: Catalyst.GridLattice{2, S}, T}, filename::String;
...
function lattice_plot(sol, sp, lrs::LatticeReactionSystem{Q, R, <: Catalyst.GridLattice{3, S}, T}; kwargs...) where {Q, R, S, T}
...
function lattice_animation(sol, sp, lrs::LatticeReactionSystem{Q, R, <: Catalyst.GridLattice{3, S}, T}, filename::String; kwargs...) where {Q, R, S, T}

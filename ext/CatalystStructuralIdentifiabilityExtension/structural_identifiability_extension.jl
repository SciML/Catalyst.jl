...
"""
function Catalyst.make_si_ode(
        rs::ReactionSystem; measured_quantities = [], known_p = [],
    ...
    measured_quantities = make_measured_quantities(
        rs, measured_quantities, known_p,
...
function SI.assess_identifiability(
    ...
    out = SI.assess_identifiability(
        osys::ODESystem, args...; measured_quantities,
        funcs_to_check, kwargs...
    )
    return make_output(out, funcs_to_check, consconsts)
end
...
function make_measured_quantities(
        rs::ReactionSystem, measured_quantities::Vector{T},
        known_p::Vector{S}, conseqs; ignore_no_measured_warn = false
    ) where {T, S}
    ...
    return Equation[
        (q isa Equation) ? q : (___internal_observables[i] ~ q)
